...
struct SaveBlock2
{
    ...
    /*0x14*/ u16 optionsTextSpeed:3; // OPTIONS_TEXT_SPEED_[SLOW/MID/FAST]
			 ...
			 u16 optionsNicknamePromptOff:1; // whether the "give a nickname?" prompt on catch is disabled
             //u16 padding1:4;
             //u16 padding2;
...
//HYDRA Egg incubator secret base decoration state. Up to HYDRA_NUM_INCUBATORS per base,
// each keyed by its base-local tile position (matches decorationPositions packing). Holds
// the incubating egg as a BoxPokemon; the egg's MON_DATA_FRIENDSHIP is the hatch countdown,
// decremented as the player walks (same rate as a party egg).
#define HYDRA_NUM_INCUBATORS 6
struct HydraIncubator
{
...
struct SaveBlock1
{
    ...
    /*0x2734*/ u8 decorationFurniture[20]; //HYDRA was decorationDesks[10]; now desks + chairs (limit 20)
    /*0x2748*/ u8 decorationMisc[20];      //HYDRA was decorationChairs[10]; now HYPER PC + egg incubator (limit 20). Adds 20 bytes to SaveBlock1; downstream offset comments below are shifted +20.
    ...
    /*0x27CA*/ struct HydraIncubator hydraIncubators[HYDRA_NUM_INCUBATORS]; //HYDRA egg incubator state (shifts fields below)
    ...
    //HYDRA quick hotbar (Select): each slot holds an ITEM_ id, or (HOTBAR_FIELDMOVE_FLAG | fieldMoveId)
    // for an HM/field move, or 0 (HOTBAR_SLOT_EMPTY). Phase 1 uses items only.
    u16 hotbar[HOTBAR_SLOTS];
