...
#include "hydra_hotbar.h" //HYDRA for HydraHotbar_IsOpen (don't clear OWEs while the bar is open)
...
//HYDRA - Object-event slots to keep free after placing an overworld encounter, so script-spawned
// NPCs / cutscene actors still have room. The encounter itself takes 1 slot, so with 16 total the
// spawn only proceeds when at least (this + 1) slots are free. Raise it if crowded maps still run
// out; lower it toward 0 to let encounters use nearly every slot.
#define OWE_OBJECT_EVENT_RESERVE        2
#define OWE_MAX_CONCURRENT              5  //HYDRA hard cap on on-screen overworld encounters (perf)
...
{
    [OWE_SPAWN_ANIM_GRASS] =    //HYDRA - use the walking rustle instead of the ledge-landing puff
    {
        .xOffset = 0,
        .yOffset = 8,
        .visual = FLDEFFOBJ_TALL_GRASS,
    },
...
void UpdateOverworldWildEncounter(void)
{
    ...
    //HYDRA - Bail out gracefully if the object-event array is (near) full rather than letting
    // SpawnSpecialObjectEvent fail its assert (the "TOO MANY OBJECT EVENTS" crash). Counted AFTER
    // the despawn above, so a recycled slot is included. Try again after the minimum spawn timer.
    {
        u32 freeSlots = OBJECT_EVENTS_COUNT;
        u32 activeOWEs = 0; //HYDRA count on-screen overworld encounters for the cap
        for (u32 i = 0; i < OBJECT_EVENTS_COUNT; i++)
        {
            if (gObjectEvents[i].active)
            {
                freeSlots--;
                if (gObjectEvents[i].trainerType == TRAINER_TYPE_OW_WILD_ENCOUNTER)
                    activeOWEs++;
            }
        }
        if (freeSlots <= OWE_OBJECT_EVENT_RESERVE // need 1 for the encounter + the reserve
         || activeOWEs >= OWE_MAX_CONCURRENT)     //HYDRA cap concurrent OWEs to bound per-frame cost
        {
            SetMinimumOWESpawnTimer();
            return;
        }
...
{
    //HYDRA don't clear spawned overworld encounters while the quick hotbar is open. Opening the bar
    // locks field controls, and some lock/UI path was wiping every spawned mon on open. The legit
    // callers (DexNav GUI exit, repel step, crossing into a town) can't run while the bar holds
    // controls, so suppressing here only stops the unwanted clear -- normal despawns resume on close.
    if (HydraHotbar_IsOpen())
        return;
