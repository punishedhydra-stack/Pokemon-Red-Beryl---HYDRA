...
#include "battle_tint.h" //HYDRA time/weather battle-background tint
#include "bg.h"
#include "data.h"
...
#include "constants/opponents.h" //HYDRA per-trainer battle background override
#include "constants/maps.h"      //HYDRA per-map (route) override: MAP_GROUP/MAP_NUM
...
static void LoadBattleEnvironmentGfx(u16 environment)
{
    ...
    LoadTintedBattleEnvPalette(gBattleEnvironmentInfo[environment].palette); //HYDRA time/weather tint (was raw LoadPalette)
}
...
//HYDRA Per-trainer battle-background override. Add a case here to force a specific battle
// background for a given trainer, regardless of the map/terrain. Return
// BATTLE_ENVIRONMENT_COUNT to mean "no override" (fall back to the normal terrain logic).
static u32 GetTrainerBattleEnvironmentOverride(u16 trainerId)
{
    switch (trainerId)
    {
    ...
    case TRAINER_CINDY_6:
        return BATTLE_ENVIRONMENT_LONG_GRASS; //HYDRA temporary - Lady Cindy on grass
    default:
        return BATTLE_ENVIRONMENT_COUNT; // no override
    }
}

//HYDRA Per-map (route-level) battle-background override. Add a map here to force a specific
// battle background for ALL battles (wild AND trainer) on that map, regardless of terrain.
// Return BATTLE_ENVIRONMENT_COUNT for "no override" (fall back to the normal terrain logic).
static u32 GetMapBattleEnvironmentOverride(void)
{
    u8 group = gSaveBlock1Ptr->location.mapGroup;
    u8 num = gSaveBlock1Ptr->location.mapNum;

    if (group == MAP_GROUP(MAP_ROUTE104) && num == MAP_NUM(MAP_ROUTE104))
        return BATTLE_ENVIRONMENT_LONG_GRASS; //HYDRA temporary - Route 104 is outdoors

    if (group == MAP_GROUP(MAP_PETALBURG_WOODS) && num == MAP_NUM(MAP_PETALBURG_WOODS))
        return BATTLE_ENVIRONMENT_GRASS; //HYDRA Petalburg Woods -> forest grass backdrop (James etc.)

    return BATTLE_ENVIRONMENT_COUNT;
}
...
static u8 GetBattleEnvironmentOverride(void)
{
    ...
    {
        u32 trainerOverride = GetTrainerBattleEnvironmentOverride(TRAINER_BATTLE_PARAM.opponentA); //HYDRA per-trainer override
        if (trainerOverride != BATTLE_ENVIRONMENT_COUNT)
    ...
    //HYDRA per-map (route-level) override - applies to wild + trainer battles on that map
    {
        u32 mapOverride = GetMapBattleEnvironmentOverride();
        if (mapOverride != BATTLE_ENVIRONMENT_COUNT)
            return mapOverride;
    }
...
bool8 LoadChosenBattleElement(u8 caseId)
{
    ...
    case 4:
        ...
        break;
    case 5:
        LoadTintedBattleEnvPalette(gBattleEnvironmentInfo[GetBattleEnvironmentOverride()].palette); //HYDRA time/weather tint (was raw LoadPalette)
        break;
    case 6:
        LoadBattleMenuWindowGfx();
        break;
