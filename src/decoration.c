...
#define DECOR_MENU_REORGANIZE 3 //HYDRA

#define DECOR_ITEMS_MENU_PLACE 0
#define DECOR_ITEMS_MENU_PUT_AWAY 1
#define DECOR_ITEMS_MENU_REORGANIZE 2 //HYDRA
...
EWRAM_DATA static u8 sCurDecorationCategory = DECORCAT_FURNITURE;
...
EWRAM_DATA static u8 sReorganizeDecorId = DECOR_NONE; //HYDRA - decoration currently being moved by RE-ORGANIZE
...
static void DecorationMenuAction_Reorganize(u8 taskId); //HYDRA
static void DecorationMenuAction_Toss(u8 taskId);
static void DecorationMenuAction_Cancel(u8 taskId);
// HYDRA RE-ORGANIZE
static void AttemptPickUpDecoration(u8 taskId);
static void AttemptPickUpDecoration_(u8 taskId);
static void MarkSingleDecorUnderCursorForPickup(u8 taskId);
static void PickUpDecorationPrompt(u8 taskId);
static void StartMovingDecoration(u8 taskId);
static void Task_LiftReorganizedDecoration(u8 taskId);
static void FieldCB_PlaceReorganizedDecoration(void);
static void Task_InitPlaceReorganizedDecoration(u8 taskId);
static void FieldCB_ReenterReorganizeSelection(void);
static void Task_InitReenterReorganizeSelection(u8 taskId);
...
{
    gText_Furniture, //HYDRA was gText_Desk
    gText_Misc,      //HYDRA was gText_Chair
    gText_Plant,
    gText_Ornament,
    gText_Mat,
    gText_Poster,
...
static const struct MenuAction sDecorationMainMenuActions[] =
{
    ...
    {
        .text = gText_Reorganize, //HYDRA
        .func = { .void_u8 = DecorationMenuAction_Reorganize },
    },
    {
        .text = gText_Toss2,
...
static const u8 *const sSecretBasePCMenuItemDescriptions[] =
{
    ...
    gText_MoveDecorationsAround, //HYDRA
...
{
    [DECOR_ITEMS_MENU_PLACE] = {
        .yesFunc = AttemptPlaceDecoration,
        .noFunc = AttemptCancelPlaceDecoration,
    },
    [DECOR_ITEMS_MENU_PUT_AWAY] = {
        .yesFunc = AttemptPutAwayDecoration,
        .noFunc = AttemptCancelPutAwayDecoration,
    },
    [DECOR_ITEMS_MENU_REORGANIZE] = { //HYDRA
        .yesFunc = AttemptPickUpDecoration,
        .noFunc = AttemptCancelPutAwayDecoration,
    },
};

static const struct YesNoFuncTable sPickUpDecorationYesNoFunctions = //HYDRA
{
    .yesFunc = StartMovingDecoration,
...
static void DecorationMenuAction_Decorate(u8 taskId)
{
    ...
    else
    {
        ...
        sCurDecorationCategory = DECORCAT_FURNITURE;
...
// HYDRA - RE-ORGANIZE: like PUT AWAY, enters the white-box field cursor, but pressing A
// on a decoration lifts it and drops the player into the normal placement flow to set it
// back down. Shares PUT AWAY's fade/warp transitions (see DECOR_MENU_REORGANIZE branches).
static void DecorationMenuAction_Reorganize(u8 taskId)
{
    if (!HasDecorationsInUse(taskId))
    {
        StringExpandPlaceholders(gStringVar4, gText_NoDecorationsInUse);
    ...
    {
        gTasks[taskId].tDecorationMenuCommand = DECOR_MENU_REORGANIZE;
        RemoveDecorationWindow(WINDOW_MAIN_MENU);
        ClearDialogWindowAndFrame(0, FALSE);
        FadeScreen(FADE_TO_BLACK, 0);
        gTasks[taskId].tState = 0;
...
static bool8 CanPlaceDecoration(u8 taskId, const struct Decoration *decoration)
{
    ...
    case DECORPERM_SPRITE:
        ...
        for (j = 0; j < mapX; j++)
        {
            curX = gTasks[taskId].tCursorX + j;
            behaviorAt = MapGridGetMetatileBehaviorAt(curX, curY);
            //HYDRA The egg incubator is an animated sprite decoration (so it must be
            // DECORPERM_SPRITE), but we want it placeable on the bare floor like the
            // HYPER PC instead of only on furniture tiles. Validate it against the floor
            // behavior, same as the metatile floor decorations do.
            if (decoration->id == DECOR_EGG_INCUBATOR)
            {
                if (!IsFloorOrBoardAndHole(behaviorAt, decoration))
                    return FALSE;
            }
            else if (decoration->id == DECOR_RADIO) //HYDRA radio: placeable on the bare floor OR on a small table/desk
            {
                if (!IsFloorOrBoardAndHole(behaviorAt, decoration)
                 && !MetatileBehavior_HoldsSmallDecoration(behaviorAt))
                    return FALSE;
            }
            else if (decoration->id >= DECOR_RED_BED && decoration->id <= DECOR_PINK_BED) //HYDRA healing beds: placeable anywhere on the bare floor
            {
                if (!IsFloorOrBoardAndHole(behaviorAt, decoration))
                    return FALSE;
            }
            else if (decoration->shape == DECORSHAPE_1x2)
            {
...
{
    //HYDRA secondary-tileset palettes (>= NUM_PALS_IN_PRIMARY) live in the secondary tileset (HYPER PC uses palette 12)
    if (pal >= NUM_PALS_IN_PRIMARY)
        CpuFastCopy(&gTilesetPointer_SecretBaseRedCave->palettes[pal], dest, PLTT_SIZE_4BPP);
    else
...
static void CopyTile(u8 *dest, u16 tile)
{
    ...
    //HYDRA read secondary-tileset tiles (>= NUM_TILES_IN_PRIMARY) from the secondary tileset so HYPER PC previews correctly
    if (tile >= NUM_TILES_IN_PRIMARY)
        CpuFastCopy(&gTilesetPointer_SecretBaseRedCave->tiles[(tile - NUM_TILES_IN_PRIMARY) * TILE_SIZE_4BPP / sizeof(u32)], buffer, TILE_SIZE_4BPP);
    else
...
// HYDRA RE-ORGANIZE - Function Implementations

static void AttemptPickUpDecoration(u8 taskId)
{
    gTasks[taskId].tButton = 0;
    ResetCursorMovement();
    AttemptPickUpDecoration_(taskId);
}
...
static void AttemptPickUpDecoration_(u8 taskId)
{
    ...
    MarkSingleDecorUnderCursorForPickup(taskId);
    if (sReorganizeDecorId != DECOR_NONE)
    {
        StringExpandPlaceholders(gStringVar4, gText_PlaceItHere);
        DisplayItemMessageOnField(taskId, gStringVar4, PickUpDecorationPrompt);
    }
    else
    {
        data = gTasks[taskId].data;
...
static void MarkSingleDecorUnderCursorForPickup(u8 taskId)
{
    ...
    sReorganizeDecorId = DECOR_NONE;
    for (i = 0; i < sDecorationContext.size; i++)
    {
        decor = sDecorationContext.items[i];
        if (decor != DECOR_NONE)
        {
            SetDecorRearrangementShape(decor, &sDecorRearrangementDataBuffer[0]);
            if (DecorationIsUnderCursor(taskId, i, &sDecorRearrangementDataBuffer[0]) == TRUE)
            {
                sDecorRearrangementDataBuffer[0].idx = i;
                sReorganizeDecorId = decor;
                sCurDecorSelectedInRearrangement = 1;
                
                // Store the decoration's current position
                sCurDecorMapX = sDecorationContext.pos[i] >> 4;
                sCurDecorMapY = sDecorationContext.pos[i] & 0x0F;
                break;
            }
        }
    }
...
{
    DisplayYesNoMenuDefaultYes();
    DoYesNoFuncWithChoice(taskId, &sPickUpDecorationYesNoFunctions);
}
...
static void StartMovingDecoration(u8 taskId)
{
    ...
    // Remove the decoration from its current position
    if (gDecorations[sReorganizeDecorId].permission != DECORPERM_SPRITE)
    {
        // For non-sprite decorations, clear the metatiles
        u8 x = sCurDecorMapX;
        u8 y = sCurDecorMapY;
        u8 width = sDecorRearrangementDataBuffer[0].width;
        u8 height = sDecorRearrangementDataBuffer[0].height;
        u8 i, j;
        ...
        for (j = 0; j < height; j++)
        {
            for (i = 0; i < width; i++)
            {
                MapGridSetMetatileEntryAt(x + MAP_OFFSET + i, y + MAP_OFFSET - j, 
                    gMapHeader.mapLayout->map[x + i + gMapHeader.mapLayout->width * (y - j)] | 0x3000);
            }
        }
    }
    
    // Set up for placing the decoration
    sPlaceDecorationGraphicsDataBuffer.decoration = &gDecorations[sReorganizeDecorId];
    gTasks[taskId].tDecorationItemsMenuCommand = DECOR_ITEMS_MENU_REORGANIZE;
    gTasks[taskId].func = Task_LiftReorganizedDecoration;
}
...
static void Task_LiftReorganizedDecoration(u8 taskId)
{
    ...
    case 1:
        RemoveFollowingPokemon();
        gPaletteFade.bufferTransferDisabled = TRUE;
        ConfigureCameraObjectForPlacingDecoration(&sPlaceDecorationGraphicsDataBuffer, sReorganizeDecorId);
        SetUpDecorationShape(taskId);
        SetUpPlacingDecorationPlayerAvatar(taskId, &sPlaceDecorationGraphicsDataBuffer);
        FadeInFromBlack();
        gPaletteFade.bufferTransferDisabled = FALSE;
...
static void FieldCB_PlaceReorganizedDecoration(void)
{
    ...
    LockPlayerFieldControls();
    FadeInFromBlack();
    taskId = CreateTask(Task_InitPlaceReorganizedDecoration, 8);
    gTasks[taskId].tState = 0;
}
...
static void Task_InitPlaceReorganizedDecoration(u8 taskId)
{
    ...
    case 3:
        if (IsWeatherNotFadingIn() == TRUE)
        {
            // Return to the reorganize selection mode
            gTasks[taskId].func = Task_SelectLocation;
        }
        break;
    }
...
static void FieldCB_ReenterReorganizeSelection(void)
{
    ...
    LockPlayerFieldControls();
    FadeInFromBlack();
    taskId = CreateTask(Task_InitReenterReorganizeSelection, 8);
    gTasks[taskId].tState = 0;
}
