...
#include "battle_anim.h" //HYDRA gBattleAnimBackgroundTable (psychic bg assets)
#include "constants/battle_anim.h" //HYDRA BG_PSYCHIC
...
//HYDRA psychic teleport transition
static void Task_HydraPsychic(u8);
static bool8 HydraPsychic_Init(struct Task *);
static bool8 HydraPsychic_SetGfx(struct Task *);
static bool8 HydraPsychic_InitScanline(struct Task *);
static bool8 HydraPsychic_Main(struct Task *);
static void VBlankCB_HydraPsychic(void);
...
static const TaskFunc sTasks_Intro[B_TRANSITION_COUNT] =
{
    ...
    //HYDRA The shared intro is what flashes the screen grey 3 times before every battle
    //HYDRA (Task_Intro -> CreateIntroTask(0, 0, 3, 2, 2) -> TransitionIntro_FadeToGray).
    //HYDRA Transition_StartIntro skips the intro entirely when this entry is NULL, so the
    //HYDRA teleport transition starts straight away with no grey flashes.
    [B_TRANSITION_HYDRA_PSYCHIC] = NULL,
};
...
static const TaskFunc sTasks_Main[B_TRANSITION_COUNT] =
{
    ...
    [B_TRANSITION_HYDRA_PSYCHIC] = Task_HydraPsychic, //HYDRA
};
...
//----------------------------------------------------------------------
//HYDRA B_TRANSITION_HYDRA_PSYCHIC
//HYDRA Full-screen psychic battle background, used by Teleport.
//HYDRA The image never moves. It animates by CYCLING ITS PALETTE, exactly as it does in battle
//HYDRA (AnimTask_SetPsychicBackground -> SetPsychicBackground_Step in battle_anim_effects_3.c).
//HYDRA No sine wave, no scanline/HBlank effect, no scrolling, and no blend ramp.
//HYDRA The 3 grey flashes are the shared transition intro and are skipped via sTasks_Intro = NULL.
//----------------------------------------------------------------------

//HYDRA how long the background is shown before fading to black
#define HYDRA_PSYCHIC_DURATION  100
//HYDRA Palette slot for the psychic background. In battle it uses GetBattleBgPaletteNum() (== 2),
//HYDRA but slot 2 belongs to the MAP out here - loading into it recoloured the map's tiles while the
//HYDRA fade was still showing them. Instead we use slot 15 (what the transition template uses) and
//HYDRA rewrite the tilemap's palette bits to match, so no map palette is ever touched.
#define HYDRA_PSYCHIC_PAL_SLOT     15
//HYDRA frames between palette rotations (matches SetPsychicBackground_Step in battle)
#define HYDRA_PSYCHIC_CYCLE_RATE    4

//HYDRA Point every tilemap entry at our palette slot (bits 12-15 of each entry are the palette
//HYDRA number). Lets us render the battle background without borrowing a palette the map is using.
static void HydraRemapPsychicTilemapPalette(u16 *tilemap)
{
    u32 i;

    for (i = 0; i < BG_SCREEN_SIZE / 2; i++)
        tilemap[i] = (tilemap[i] & 0x0FFF) | (HYDRA_PSYCHIC_PAL_SLOT << 12);
}

//HYDRA This is how the psychic background animates in battle: it does not move at all, the COLOURS
//HYDRA rotate through the pattern. Lifted from SetPsychicBackground_Step (battle_anim_effects_3.c):
//HYDRA every few frames, palette entries 1..11 are rotated by one.
static void HydraCyclePsychicPalette(u8 paletteIndex)
{
    int i;
    u16 lastColor = gPlttBufferFaded[BG_PLTT_ID(paletteIndex) + 11];
    u16 lastColorUnfaded = gPlttBufferUnfaded[BG_PLTT_ID(paletteIndex) + 11];

    for (i = 10; i > 0; i--)
    {
        gPlttBufferFaded[BG_PLTT_ID(paletteIndex) + i + 1] = gPlttBufferFaded[BG_PLTT_ID(paletteIndex) + i];
        //HYDRA Rotate the UNFADED buffer in step too. BeginNormalPaletteFade fades FROM the unfaded
        //HYDRA buffer, so if only the faded one is rotated the colours snap back to their original
        //HYDRA rotation the instant the fade-to-black starts.
    ...
    gPlttBufferUnfaded[BG_PLTT_ID(paletteIndex) + 1] = lastColorUnfaded;
}

//HYDRA frames per blend step while fading in (1 = ~16 frames to fully opaque)
#define HYDRA_PSYCHIC_FADE_RATE     1
//HYDRA VBlanks to keep BG0 hidden before revealing it. Init/SetGfx/InitScanline all run in ONE
//HYDRA frame and write a lot of VRAM during active display, so revealing BG0 too early can catch a
//HYDRA half-built or wrongly-coloured frame. 2 guarantees a clean frame first.
#define HYDRA_PSYCHIC_SHOW_DELAY    2

static u8 sHydraPsychicShowDelay;

#define tTimer        data[1]
#define tStartedFade  data[2]
#define tCycleTimer   data[3]
#define tBlendTarget2 data[4]
#define tBlendTarget1 data[5]

static const TransitionStateFunc sHydraPsychic_Funcs[] =
{
    HydraPsychic_Init,
    HydraPsychic_SetGfx,
    HydraPsychic_InitScanline,
    HydraPsychic_Main
};

static void Task_HydraPsychic(u8 taskId)
{
    while (sHydraPsychic_Funcs[gTasks[taskId].tState](&gTasks[taskId]));
}

static bool8 HydraPsychic_Init(struct Task *task)
{
    u16 *tilemap, *tileset;
    ...
    //HYDRA make sure no leftover HBlank/scanline effect distorts the image (that was the "wave")
    DisableInterrupts(INTR_FLAG_HBLANK);
    SetHBlankCallback(NULL);

    //HYDRA Switch BG0 OFF while we load it. Tiles/tilemap go straight into VRAM but a LoadPalette
    //HYDRA only reaches the hardware on the next VBlank, so for one frame the background drew with
    //HYDRA whatever was previously in the palette slot. Hiding BG0 until everything is ready kills
    //HYDRA that stray frame; VBlankCB_HydraPsychic turns it back on between frames.
    //HYDRA
    //HYDRA IMPORTANT: ClearGpuRegBits() goes through SetGpuReg(), which only writes the hardware
    //HYDRA immediately when VCOUNT is 161-225 (VBlank) or the screen is force-blanked - otherwise it
    //HYDRA just QUEUES the value until the next VBlank (see src/gpu_regs.c). This function runs during
    //HYDRA active display, so the queued write left BG0 switched ON in hardware for the whole frame in
    //HYDRA which we decompress the tileset - that is the stray frame. Keep the shadow buffer in sync
    //HYDRA via ClearGpuRegBits, then poke REG_DISPCNT directly so it actually takes effect NOW.
    ClearGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_BG0_ON);
    REG_DISPCNT &= ~DISPCNT_BG0_ON;
    sHydraPsychicShowDelay = 0;

    //HYDRA Fade the background IN rather than cutting to it. BLDALPHA_BLEND(eva, evb): eva is BG0's
    //HYDRA share, evb is everything behind it. Start at (0, 16) = map fully visible, ramp to
    //HYDRA (16, 0) = psychic fully opaque.
    task->tBlendTarget2 = 0;
    task->tBlendTarget1 = 16;
    sTransitionData->BLDCNT = BLDCNT_TGT1_BG0 | BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_ALL;
    sTransitionData->BLDALPHA = BLDALPHA_BLEND(task->tBlendTarget2, task->tBlendTarget1);
    ...
    GetBg0TilesDst(&tilemap, &tileset);
    CpuFill16(0, tilemap, BG_SCREEN_SIZE);
    DecompressDataWithHeaderVram(gBattleAnimBackgroundTable[BG_PSYCHIC].image, tileset);
    //HYDRA In battle this background uses GetBattleBgPaletteNum(), which is 2 outside contests, so the
    //HYDRA tilemap references palette 2. Slot 15 is loaded too purely as a harmless fallback; if the
    //HYDRA colours look right you can delete the _ALT line (and its cycle call in Main).
    LoadPalette(gBattleAnimBackgroundTable[BG_PSYCHIC].palette, BG_PLTT_ID(HYDRA_PSYCHIC_PAL_SLOT), PLTT_SIZE_4BPP);
    sTransitionData->cameraY = 0;

    //HYDRA Zero BG0's scroll NOW. The VBlank callback that holds it at 0 is not installed until a
    //HYDRA later phase, so without this the background inherits the field camera's leftover scroll
    //HYDRA and draws offset for the first frames.
    REG_BG0HOFS = 0;
    REG_BG0VOFS = 0;

    task->tState++;
    //HYDRA return TRUE (not FALSE) so SetGfx runs in this SAME frame. Returning FALSE ended the frame
    //HYDRA with the tilemap still cleared, which drew one solid palette-0 grey frame = a grey flash.
    return TRUE;
}

static bool8 HydraPsychic_SetGfx(struct Task *task)
{
    u16 *tilemap, *tileset;

    GetBg0TilesDst(&tilemap, &tileset);
    DecompressDataWithHeaderVram(gBattleAnimBackgroundTable[BG_PSYCHIC].tilemap, tilemap);
    HydraRemapPsychicTilemapPalette(tilemap); //HYDRA so it uses our slot, not a map palette

    task->tState++;
    return TRUE;
}

static bool8 HydraPsychic_InitScanline(struct Task *task)
{
    //HYDRA no scanline effect for this transition - just install the VBlank scroller
    SetVBlankCallback(VBlankCB_HydraPsychic);
    //HYDRA NOTE: BG0 is deliberately NOT re-enabled here. Init/SetGfx/InitScanline all return TRUE,
    //HYDRA so they run in the SAME frame - enabling it here would still be inside the frame whose
    //HYDRA palette has not reached the hardware yet. VBlankCB_HydraPsychic enables it instead, which
    //HYDRA runs between frames, after TransferPlttBuffer.

    task->tState++;
    return TRUE;
}

static bool8 HydraPsychic_Main(struct Task *task)
{
    sTransitionData->VBlank_DMA = FALSE;

    //HYDRA ramp the fade-in until the background is fully opaque
    if (task->tBlendTarget2 < 16 && task->tTimer % HYDRA_PSYCHIC_FADE_RATE == 0)
    {
        task->tBlendTarget2++;
        task->tBlendTarget1--;
        sTransitionData->BLDALPHA = BLDALPHA_BLEND(task->tBlendTarget2, task->tBlendTarget1);
    ...
    //HYDRA animate by rotating the palette (stop once the fade-to-black starts, or we would be
    //HYDRA fighting BeginNormalPaletteFade over the same buffer)
    if (!task->tStartedFade && ++task->tCycleTimer >= HYDRA_PSYCHIC_CYCLE_RATE)
    {
        HydraCyclePsychicPalette(HYDRA_PSYCHIC_PAL_SLOT);
        task->tCycleTimer = 0;
    }

    if (++task->tTimer == HYDRA_PSYCHIC_DURATION)
    {
        task->tStartedFade++;
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
    }
    ...
    if (task->tStartedFade && !gPaletteFade.active)
    {
        //HYDRA Hand the display back before we go. VBlankCB_HydraPsychic force-enables BG0 and
        //HYDRA re-applies the blend EVERY VBlank - if it keeps running after this task dies it is
        //HYDRA still doing that during the map load, blending BG0 against half-built VRAM, which
        //HYDRA shows up as a grey flash. Reset the blend and drop back to the plain callback.
        REG_BLDCNT = 0;
        REG_BLDALPHA = 0;
        sTransitionData->BLDCNT = 0;
        sTransitionData->BLDALPHA = 0;
        ClearGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_BG0_ON);
        SetVBlankCallback(VBlankCB_BattleTransition);
        DestroyTask(FindTaskIdByFunc(Task_HydraPsychic));
    }
...
{
    VBlankCB_BattleTransition();
    //HYDRA Reveal BG0 only after a couple of clean VBlanks. The tileset, tilemap and palette are all
    //HYDRA written during active display in a single frame, so showing it immediately could catch a
    //HYDRA half-built or wrongly-coloured frame.
    if (sHydraPsychicShowDelay < HYDRA_PSYCHIC_SHOW_DELAY)
        sHydraPsychicShowDelay++;
    else
        SetGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_BG0_ON);
    //HYDRA apply the fade-in blend
    REG_BLDCNT = sTransitionData->BLDCNT;
    REG_BLDALPHA = sTransitionData->BLDALPHA;
    //HYDRA hold the background steady (no scrolling)
    REG_BG0HOFS = 0;
    REG_BG0VOFS = 0;
}
