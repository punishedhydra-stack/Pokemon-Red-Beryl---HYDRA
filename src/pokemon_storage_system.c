...
enum {
    ...
    OPTIONS_COUNT,
    OPTION_SELECT_MON,
    OPTION_HYPER_TRAIN //HYDRA HYPER PC: pick a mon to hyper train or rename
};
...
enum {
    ...
    MENU_SIMPLE,
    MENU_SELECT,
    MENU_HYPER_TRAIN, //HYDRA
    MENU_NICKNAME,    //HYDRA (MENU_NAME is the box name, so this needs its own entry)
};
...
enum {
    ...
    SCREEN_CHANGE_HYPER_SUMMARY, //HYDRA
    SCREEN_CHANGE_NICKNAME_MON,  //HYDRA
};
...
//HYDRA HYPER PC: the mon picked for renaming. Held here because sStorage is freed
// before the naming screen opens, so the pointer must survive the screen change.
EWRAM_DATA static struct BoxPokemon *sHydraNicknameBoxMon = NULL;
...
static void Task_ShowHyperSummary(u8);          //HYDRA
static void Task_HydraNicknameMon(u8);          //HYDRA
static void CB2_HydraReturnFromNicknameScreen(void); //HYDRA
...
static void EnterPokeStorage(u8 boxOption)
{
    ...
    if (sStorage == NULL)
    {
        //HYDRA HYPER PC is script-invoked like OPTION_SELECT_MON, so an allocation
        // failure must return to the script rather than the PC exit callback.
        if (boxOption == OPTION_SELECT_MON || boxOption == OPTION_HYPER_TRAIN)
...
static void CB2_ReturnToPokeStorage(void)
{
    ...
    if (sStorage == NULL)
    {
        //HYDRA HYPER PC exits back to the overworld script, same as OPTION_SELECT_MON
        if (sStorage->boxOption == OPTION_SELECT_MON || sStorage->boxOption == OPTION_HYPER_TRAIN)
            SetMainCallback2(CB2_ReturnToFieldContinueScript);
        else
            SetMainCallback2(CB2_ExitPokeStorage);
    }
...
{
    ClearSavedCursorPos();
    //HYDRA HYPER PC shows the party alongside the box so the cursor can start there
    sInPartyMenu = (sStorage->boxOption == OPTION_DEPOSIT || sStorage->boxOption == OPTION_HYPER_TRAIN);
    sDepositBoxId = 0;
}
    ...
    case 0:
        ...
        if (sStorage->isReopening)
        {
            ...
            case SCREEN_CHANGE_ITEM_FROM_BAG - 1:
                ...
                GiveChosenBagItem();
                break;
            //HYDRA Returning from the hyper summary screen: restore the cursor the
            // same way the normal summary screen does.
            case SCREEN_CHANGE_HYPER_SUMMARY - 1:
                SetSelectionAfterSummaryScreen();
                break;
            //HYDRA Returning from the nickname screen: nothing to restore, the name
            // was already written in CB2_HydraReturnFromNicknameScreen.
            case SCREEN_CHANGE_NICKNAME_MON - 1:
                break;
            }
        }
...
static void Task_PokeStorageMain(u8 taskId)
{
    ...
    case MSTATE_HANDLE_INPUT:
        ...
        case INPUT_SHOW_PARTY:
            //HYDRA OPTION_HYPER_TRAIN excluded here too, so going back up to the
            // party reopens it instead of printing the "which one will take" prompt.
            if (sStorage->boxOption != OPTION_MOVE_MONS && sStorage->boxOption != OPTION_MOVE_ITEMS
             && sStorage->boxOption != OPTION_SELECT_MON && sStorage->boxOption != OPTION_HYPER_TRAIN)
            {
        ...
        case INPUT_HIDE_PARTY:
            //HYDRA OPTION_HYPER_TRAIN must be listed here or B in the party area
            // generates this input and nothing acts on it, trapping the cursor.
            if (sStorage->boxOption == OPTION_MOVE_MONS || sStorage->boxOption == OPTION_SELECT_MON
             || sStorage->boxOption == OPTION_HYPER_TRAIN)
            {
...
static void Task_OnSelectedMon(u8 taskId)
{
    ...
    case 2:
        ...
        case MENU_SUMMARY:
            ...
            break;
        //HYDRA HYPER PC options
        case MENU_HYPER_TRAIN:
            PlaySE(SE_SELECT);
            SetPokeStorageTask(Task_ShowHyperSummary);
            break;
        case MENU_NICKNAME:
            PlaySE(SE_SELECT);
            SetPokeStorageTask(Task_HydraNicknameMon);
            break;
        case MENU_MARK:
            PlaySE(SE_SELECT);
...
//HYDRA HYPER PC: open the hyper summary screen for the highlighted mon. Mirrors
// Task_ShowMonSummary exactly, only the screen change type differs.
static void Task_ShowHyperSummary(u8 taskId)
{
    switch (sStorage->state)
    {
    case 0:
    ...
    case 1:
        if (!UpdatePaletteFade())
        {
            sWhichToReshow = SCREEN_CHANGE_HYPER_SUMMARY - 1;
            sStorage->screenChangeType = SCREEN_CHANGE_HYPER_SUMMARY;
            SetPokeStorageTask(Task_ChangeScreen);
        }
        break;
    }
...
//HYDRA HYPER PC: rename the highlighted mon, then come back to the box.
static void Task_HydraNicknameMon(u8 taskId)
{
    switch (sStorage->state)
    {
    case 0:
        sHydraNicknameBoxMon = GetCursorBoxMon();
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
        sStorage->state++;
        break;
    case 1:
        if (!UpdatePaletteFade())
        {
            sWhichToReshow = SCREEN_CHANGE_NICKNAME_MON - 1;
            sStorage->screenChangeType = SCREEN_CHANGE_NICKNAME_MON;
            SetPokeStorageTask(Task_ChangeScreen);
        }
        break;
    }
...
//HYDRA Write the entered nickname back to the mon and return to the box.
static void CB2_HydraReturnFromNicknameScreen(void)
{
    if (sHydraNicknameBoxMon != NULL)
        SetBoxMonData(sHydraNicknameBoxMon, MON_DATA_NICKNAME, gStringVar2);
...
static void Task_ChangeScreen(u8 taskId)
{
    ...
    case SCREEN_CHANGE_ITEM_FROM_BAG:
        ...
        break;
    //HYDRA HYPER PC: same pattern as the summary screen, but opens the hyper
    // summary and returns to the box afterwards.
    case SCREEN_CHANGE_HYPER_SUMMARY:
        boxMons = sStorage->summaryMon.box;
        monIndex = sStorage->summaryStartPos;
        maxMonIndex = sStorage->summaryMaxPos;
        mode = sStorage->summaryScreenMode;
        FreePokeStorageData();
        ShowHyperSummaryScreen(mode, boxMons, monIndex, maxMonIndex, CB2_ReturnToPokeStorage);
        break;
    //HYDRA HYPER PC: rename the selected mon, then return to the box.
    case SCREEN_CHANGE_NICKNAME_MON:
    {
        struct BoxPokemon *boxMon = sHydraNicknameBoxMon;
        u16 nickSpecies = GetBoxMonData(boxMon, MON_DATA_SPECIES);
        u16 nickGender = GetBoxMonGender(boxMon);
        u32 nickPersonality = GetBoxMonData(boxMon, MON_DATA_PERSONALITY);

        GetBoxMonData(boxMon, MON_DATA_NICKNAME, gStringVar2);
        FreePokeStorageData();
        DoNamingScreen(NAMING_SCREEN_NICKNAME, gStringVar2, nickSpecies, nickGender,
                       nickPersonality, CB2_HydraReturnFromNicknameScreen);
        break;
    }
    }
...
static void InitCursor(void)
{
    //HYDRA HYPER PC starts on the first party slot (box mons are still reachable)
    if (sStorage->boxOption == OPTION_DEPOSIT || sStorage->boxOption == OPTION_HYPER_TRAIN)
...
static bool8 SetMenuTexts_Mon(void)
{
    ...
    //HYDRA HYPER PC offers only these three, so it returns before the shared
    // SUMMARY / MARK / RELEASE block below.
    if (sStorage->boxOption == OPTION_HYPER_TRAIN)
    {
        if (species == SPECIES_NONE)
            return FALSE;

        // Until the 3rd badge the HYPER PC can only rename, so HYPER TRAIN is
        // hidden rather than opening a screen where nothing can be edited.
        if (HyperPC_CanEditAnything())
            SetMenuText(MENU_HYPER_TRAIN);
...
static const u8 *const sMenuTexts[] =
{
    ...
    [MENU_HYPER_TRAIN] = COMPOUND_STRING("HYPER TRAIN"), //HYDRA
    [MENU_NICKNAME]    = COMPOUND_STRING("NICKNAME"),    //HYDRA
};
...
//HYDRA Opens the PC for the HYPER PC decoration. Cursor starts on the first party
// slot; box mons are still reachable. Selecting a mon offers HYPER TRAIN /
// NICKNAME / CANCEL.
void ChooseMonForHyperTraining(void)
{
    EnterPokeStorage(OPTION_HYPER_TRAIN);
}
