...
#include "hydra_hotbar.h" //HYDRA quick hotbar register/pick integration
...
static void GetItemNameFromPocket(u8 *dest, enum Item itemId)
{
    ...
    case POCKET_BERRIES:
        ...
        break;
    default:
        // ADD THIS CHECK for Refill Potion
        if (IsVialItem(itemId)) //HYDRA
        {
            u8 charges = gSaveBlock1Ptr->refillablePotionCharges;
            u8 maxCharges = GetVialMaxCharges();
            CopyItemName(itemId, gStringVar1);
            ConvertIntToDecimalStringN(gStringVar2, charges, STR_CONV_MODE_LEFT_ALIGN, 2);
            ConvertIntToDecimalStringN(gStringVar3, maxCharges, STR_CONV_MODE_LEFT_ALIGN, 2);
            StringExpandPlaceholders(dest, COMPOUND_STRING("{STR_VAR_1} ({STR_VAR_2}/{STR_VAR_3})"));
        }
        else
        {
        ...
        default: // A_BUTTON
            {
                ...
                //HYDRA quick hotbar: if the bag was opened from an empty hotbar slot, A registers
                // the highlighted key item into that slot and returns to the field (way 2).
                if (HydraHotbar_IsPicking() == TRUE)
                {
                    if (gBagPosition.pocket == POCKET_KEY_ITEMS && itemSlot.itemId != ITEM_NONE)
                    {
                        PlaySE(SE_SELECT);
                        HydraHotbar_TryAssignPicked(itemSlot.itemId);
                        gSpecialVar_ItemId = itemSlot.itemId;
                        gTasks[taskId].func = Task_FadeAndCloseBagMenu;
                    }
                    else if (gBagPosition.pocket == POCKET_TM_HM && HydraHotbar_TryAssignPickedHM(itemSlot.itemId) == TRUE)
                    {
                        PlaySE(SE_SELECT);
                        gSpecialVar_ItemId = itemSlot.itemId;
                        gTasks[taskId].func = Task_FadeAndCloseBagMenu;
                    ...
                    {
                        PlaySE(SE_BOO); // not registerable here (needs a key item, or an HM you can use)
                    }
                    break;
                }
                PlaySE(SE_SELECT);
...
static void ItemMenu_Register(u8 taskId)
{
    ...
    //HYDRA quick hotbar: from the field bag, Register queues this item and reopens the hotbar
    // so the player picks which of the 8 slots to drop it into (way 1).
    if (gBagPosition.location == ITEMMENULOCATION_FIELD)
    {
        HydraHotbar_SetPendingAssign(gSpecialVar_ItemId);
        RemoveContextWindow();
        gBagPosition.exitCallback = CB2_HydraHotbar_ReturnToField;
        Task_FadeAndCloseBagMenu(taskId);
        return;
    }
...
static void ItemMenu_UseInBattle(u8 taskId) //HYDRA
{
    // Safety check
    ...
    // Block items in trainer battles
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
        RemoveContextWindow();
        StringCopy(gStringVar4, gBattleStringsTable[STRINGID_NOITEMSINTRAINERBATTLE]);
        DisplayItemMessage(taskId, FONT_SHORT, gStringVar4, HandleErrorMessage);
        return;
    }
