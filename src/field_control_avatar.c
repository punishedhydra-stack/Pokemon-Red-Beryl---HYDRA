...
#include "daycare.h"
#include "hydra_incubator.h" //HYDRA egg incubator step advance
#include "debug.h"
#include "dexnav.h"
...
#include "hydra_hotbar.h" //HYDRA quick hotbar (SELECT)
...
int ProcessPlayerFieldInput(struct FieldInput *input)
{
    ...
    if (input->pressedSelectButton) //HYDRA SELECT opens the quick hotbar instead of the single registered item
    {
        HydraHotbar_OpenFromField();
        return TRUE;
    }
...
static bool8 TryStartInteractionScript(struct MapPosition *position, u16 metatileBehavior, enum Direction direction)
{
    ...
    if (script != LittlerootTown_BrendansHouse_2F_EventScript_PC
     ...
     && script != SecretBase_EventScript_HyperTrainingPC //HYDRA
...
static const u8 *GetInteractedObjectEventScript(struct MapPosition *position, u8 metatileBehavior, enum Direction direction)
{
    ...
    if (PlayerHasFollowerNPC() && objectEventId == GetFollowerNPCObjectId())
        ...
        script = GetOverworlWildEncounterScript(objectEventId);
          ...
          || gObjectEvents[objectEventId].localId == OBJ_EVENT_ID_FOLLOWER_2)
...
static const u8 *GetInteractedMetatileScript(struct MapPosition *position, u8 metatileBehavior, enum Direction direction)
{
    ...
    if (elevation == MapGridGetElevationAt(position->x, position->y))
    {
        ...
        //HYDRA HYPER PC decoration interaction
        if (MetatileBehavior_IsSecretBaseHyperTrainingPC(metatileBehavior) == TRUE)
            return SecretBase_EventScript_HyperTrainingPC;
...
static const u8 *GetInteractedWaterScript(struct MapPosition *unused1, u8 metatileBehavior, enum Direction direction)
{
    ...
    //HYDRA Also auto-surf on A when a compatible mon + the Surf HM in the bag + the
    // Balance Badge qualify, even if no mon actually knows Surf (matches the party menu).
    if (IsFieldMoveUnlocked(FIELD_MOVE_SURF)
     && (PartyHasMonWithSurf() == TRUE || CanUseHMFieldMoveFromBag(FIELD_MOVE_SURF))
...
static bool8 TryStartStepCountScript(u16 metatileBehavior)
{
    ...
    HydraIncubator_Step(); //HYDRA advance eggs sitting in secret base incubators
