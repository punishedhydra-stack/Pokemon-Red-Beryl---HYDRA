...
void ShowPokemonSummaryScreen(u8 mode, void *mons, u8 monIndex, u8 maxMonIndex, void (*callback)(void))
{
    ...
    case SUMMARY_MODE_SELECT_MOVE:
        sMonSummaryScreen->minPageIndex = PSS_PAGE_SKILLS; //HYDRA allow sliding left to Skills to check stats while learning a move
        sMonSummaryScreen->maxPageIndex = PSS_PAGE_COUNT - 1;
    ...
    else if (mode == SUMMARY_MODE_SELECT_MOVE) //HYDRA open on Battle Moves even though min is now Skills
        sMonSummaryScreen->currPageIndex = PSS_PAGE_BATTLE_MOVES;
    else
...
static void PssScrollRight(u8 taskId) // display left
{
    s16 *data = gTasks[taskId].data;
    ...
    if (data[0] == 0)
    {
        ...
        // Fix for MoveSelectMode: set the source BG to the current page
        // so it doesn't show Skills from a previous scroll
        if (sMonSummaryScreen->mode == SUMMARY_MODE_SELECT_MOVE)
        {
            u8 srcBg = (data[1] == 1) ? 2 : 1;
            SetBgTilemapBuffer(srcBg, sMonSummaryScreen->bgTilemapBuffers[sMonSummaryScreen->currPageIndex - 1][0]);
            ChangeBgX(srcBg, 0, BG_COORD_SET);
        }
...
static void PssScrollRightEnd(u8 taskId) // display right
{
    ...
    // Fix for MoveSelectMode: set the "other" BG to the current page
    // so it's ready as the source for the next scroll
    if (sMonSummaryScreen->mode == SUMMARY_MODE_SELECT_MOVE)
    {
        u8 otherBg = (data[1] == 1) ? 2 : 1;
        SetBgTilemapBuffer(otherBg, sMonSummaryScreen->bgTilemapBuffers[sMonSummaryScreen->currPageIndex][0]);
        ChangeBgX(otherBg, 0x10000, BG_COORD_SET);
    }
...
static void PssScrollLeft(u8 taskId) // display right
{
    s16 *data = gTasks[taskId].data;
    ...
    if (data[0] == 0)
    {
        ...
        {
            u8 srcBg = (data[1] == 1) ? 2 : 1;
            SetBgTilemapBuffer(srcBg, sMonSummaryScreen->bgTilemapBuffers[sMonSummaryScreen->currPageIndex][0]);
            ChangeBgX(srcBg, 0, BG_COORD_SET);
        }
...
static void PssScrollLeftEnd(u8 taskId) // display left
{
    ...
    // Fix for MoveSelectMode: set the "other" BG to the current page
    // instead of the previous page (which would be Skills)
    if (sMonSummaryScreen->mode == SUMMARY_MODE_SELECT_MOVE)
    {
        u8 otherBg = (data[1] == 1) ? 2 : 1;
        SetBgTilemapBuffer(otherBg, sMonSummaryScreen->bgTilemapBuffers[sMonSummaryScreen->currPageIndex][0]);
    ...
    else if (sMonSummaryScreen->currPageIndex > 1)
    {
        // Original code for non-MoveSelectMode
...
static void Task_HandleReplaceMoveInput(u8 taskId)
{
    ...
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE)
    {
        ...
        {
            //HYDRA Only allow move selection (up/down) and confirming a replace (A) while on a
            // move page. Left/right still pages over to Skills to check stats mid-learn.
            bool32 onMovePage = IS_MOVE_PAGE(sMonSummaryScreen->currPageIndex);

            if (onMovePage && JOY_NEW(DPAD_UP))
            {
                data[0] = 4;
                ChangeSelectedMove(data, -1, &sMonSummaryScreen->firstMoveIndex);
            }
            else if (onMovePage && JOY_NEW(DPAD_DOWN))
            {
                data[0] = 4;
                ChangeSelectedMove(data, 1, &sMonSummaryScreen->firstMoveIndex);
            }
            ...
            else if (onMovePage && JOY_NEW(A_BUTTON)) //HYDRA only confirm a replace from a move page
            {
...
static void PutPageWindowTilemaps(u8 page)
{
    ...
    case PSS_PAGE_INFO:
        ...
        //HYDRA The info page no longer has a prompt (A + RENAME/CANCEL removed).
        // ShowUtilityPrompt clears that window, but this used to put it straight
        // back with the stale pixels still in VRAM, so the icon and text kept
        // showing. The skills page guards its equivalent call the same way.
...
{
    //HYDRA Keep the move cursor hidden on non-move pages so it doesn't bleed onto the
    // Skills page when sliding over to check stats during move learning.
...
{
    //HYDRA RELEARN TM prompt removed from the normal summary screen. This is the
    // single gate for the prompt window and the START-button handler, so returning
    // FALSE removes both. File-local: hyper_summary_screen.c is unaffected.
    return FALSE;
}

static inline bool32 ShouldShowRename(void)
{
    //HYDRA Rename removed from the normal summary screen; renaming lives on the
    // HYPER PC now. This is the single gate for both the prompt and its A-button
    // handler, so returning FALSE removes the option entirely and the info page
    // falls back to the vanilla CANCEL prompt.
    // File-local: hyper_summary_screen.c keeps its own copy for the EDIT prompt.
    return FALSE;
}
...
static inline void ShowUtilityPrompt(s16 mode)
{
    ...
    //HYDRA The info page prompt (the A icon plus RENAME/CANCEL) is removed. Leaving
    // promptText NULL makes the code further down clear the window, blanking that
    // corner. B still closes the screen, it just is not advertised any more.
    if (sMonSummaryScreen->currPageIndex == PSS_PAGE_SKILLS)
    {
        if (ShouldShowIvEvPrompt())
        {
            //HYDRA This used to show the NEXT mode in the cycle (a "press to switch
            // to X" hint), which read as the page being mislabelled -- viewing EVs
            // said STATS, viewing Stats said IVs, and so on. Now it names the mode
            // you are actually looking at, matching the in-panel label.
            if (mode == SUMMARY_SKILLS_MODE_STATS)
