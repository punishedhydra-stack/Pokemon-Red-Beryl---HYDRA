...
#if !FREE_LINK //HYDRA Pokemon Jump gutted (link minigame)
...
#else //HYDRA stubs (FREE_LINK): keep only symbols referenced elsewhere
void StartPokemonJump(u16 partyId, MainCallback exitCallback) { SetMainCallback2(exitCallback); }
bool32 IsSpeciesAllowedInPokemonJump(enum Species species) { return FALSE; }
void IsPokemonJumpSpeciesInParty(void) { gSpecialVar_Result = FALSE; }
void ResetPokemonJumpRecords(void) {}
void ShowPokemonJumpRecords(void) {}
#endif //HYDRA !FREE_LINK