...
#include "battle_weather.h" //HYDRA gInBattleWeatherDamage: keep the weather overlay up through the damage
...
static bool32 HandleEndTurnWeatherDamage(enum BattlerId battler)
{
    ...
    case BATTLE_WEATHER_SANDSTORM:
         ...
         && !IsAbilityAndRecord(battler, ability, ABILITY_MAGIC_GUARD))
        {
            ...
            gInBattleWeatherDamage = TRUE; //HYDRA keep the weather overlay visible before/during/after the sandstorm damage
    ...
    case BATTLE_WEATHER_SNOW:
        ...
        else if (currBattleWeather == BATTLE_WEATHER_HAIL)
        {
             ...
             && !IsAbilityAndRecord(battler, ability, ABILITY_MAGIC_GUARD))
            {
                ...
                gInBattleWeatherDamage = TRUE; //HYDRA keep the weather overlay visible before/during/after the hail damage
                BattleScriptCall(BattleScript_DamagingWeather);
                effect = TRUE;
            }
        }
