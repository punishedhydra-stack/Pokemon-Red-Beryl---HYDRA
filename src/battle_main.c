...
#include "battle_weather.h" //HYDRA continuous in-battle weather overlay
...
extern u16 gStolenItems[];
extern u8 gStolenItemCount;
extern u16 gStolenItems[6];
extern u8 gStolenItemCount;
...
static void CB2_InitBattleInternal(void)
{
    ...
    ResetSpriteData(); //HYDRA moved this up to deal with sprite slot issue
    ResetTasks();
    ...
    if (!DEBUG_OVERWORLD_MENU || (DEBUG_OVERWORLD_MENU && !gIsDebugBattle))
    {
        if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED)))
        {
			...
			if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) //HYDRA
             && !(gBattleTypeFlags & (BATTLE_TYPE_TWO_OPPONENTS | BATTLE_TYPE_INGAME_PARTNER))
             && GetMonData(&gParties[B_TRAINER_OPPONENT_A][0], MON_DATA_SANITY_HAS_SPECIES)
             && !GetMonData(&gParties[B_TRAINER_OPPONENT_A][1], MON_DATA_SANITY_HAS_SPECIES))
			{
                CopyMon(&gParties[B_TRAINER_OPPONENT_A][1], &gParties[B_TRAINER_OPPONENT_A][0], sizeof(struct Pokemon));
                CalculateEnemyPartyCount();
            }
        }
        if (gBattleTypeFlags & BATTLE_TYPE_MULTI)
...
void BattleMainCB2(void)
{
    ...
    UpdatePaletteFade();
    RunTasks();
    UpdateBattleWeatherOverlay(); //HYDRA keep continuous weather overlay alive (start-of-battle + after sub-screens)
...
u8 GetPlayerPartyHighestLevel(void) //HYDRA
{
    u8 highestLevel = 0;
    u32 i;
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_SANITY_HAS_SPECIES) //HYDRA - was gPlayerParty (deprecated)
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 halfTeam, u32 battleTypeFlags)
{
    ...
    u8 monsCount;
	u8 scaledLevel = GetPlayerPartyHighestLevel(); //HYDRA
	bool32 skipScaling = (trainer->trainerClass == TRAINER_CLASS_LEADER
	                   || trainer->trainerClass == TRAINER_CLASS_ELITE_FOUR
	                   || trainer->trainerClass == TRAINER_CLASS_CHAMPION); //HYDRA
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (s32 i = 0; i < monsCount; i++)
        {
            ...
            u8 monLevel = skipScaling ? partyData[monIndex].lvl : scaledLevel; //HYDRA
            CreateMon(&party[i], partyData[monIndex].species, monLevel, personalityValue, otId); //HYDRA
...
void SpriteCB_PlayerMonSlideIn(struct Sprite *sprite)
{
    if (sprite->data[3] == 0)
    {
        ...
        if (sprite->animEnded)
            StartSpriteAnim(sprite, 0);
        sprite->x2 = -220;          //HYDRA - offset off-screen left using x2 (the field that moves battle sprites)
        sprite->invisible = FALSE;
        sprite->data[3]++;
    }
    else if (sprite->x2 < 0)        //HYDRA - slide x2 back to 0
    {
        sprite->x2 += 8;
        if (sprite->x2 > 0)
            sprite->x2 = 0;
    ...
    {
        // Wait out any cry already playing (the partner's) before handing off. This is
        // what lets the two entrances overlap visually while the cries still take turns.
        // Holding the whole hand-off, rather than just the cry, keeps this mon's
        // animation and its cry in sync -- the sprite simply rests on its landing spot
        // for a moment. x2 is already clamped to 0 by the slide above, so nothing moves.
...
static void BattleStartClearSetData(void)
{
	...
	memset(gStolenItems, ITEM_NONE, sizeof(gStolenItems));
	gStolenItemCount = 0;
    ...
    // Clear stolen items tracking from previous battle
    memset(gStolenItems, ITEM_NONE, sizeof(gStolenItems));
    gStolenItemCount = 0;
...
//HYDRA true while the battle is in the action/move selection phase (the Fight/Bag/Pokemon/Run
// menu, including move select) -- used by the in-battle rain overlay to play rain audio only then.
bool32 IsBattleActionSelectionActive(void)
{
    return gBattleMainFunc == HandleTurnActionSelectionState;
}

//HYDRA true during the whole pre-first-turn intro -- the send-out sequence (DoBattleIntro) AND the
// before-first-turn events (TryDoEventsBeforeFirstTurn). Both are safe idle phases (nothing can
// faint), so the rain overlay is allowed to show there: rain is visible from the very start of the
// battle, not only once the action menu appears.
bool32 IsBattleBeforeFirstTurn(void)
{
    return gBattleMainFunc == DoBattleIntro || gBattleMainFunc == TryDoEventsBeforeFirstTurn;
}
...
static void HandleEndTurn_FinishBattle(void)
{
    if (gCurrentActionFuncId == B_ACTION_TRY_FINISH || gCurrentActionFuncId == B_ACTION_FINISHED)
    {
        ...
        // Force all battler sprites to complete their animations before fade
        for (u32 i = 0; i < MAX_BATTLERS_COUNT; i++)
        {
            if (gBattlerSpriteIds[i] < MAX_SPRITES && gSprites[gBattlerSpriteIds[i]].inUse)
            {
        ...
        if (B_TRAINERS_KNOCK_OFF_ITEMS == TRUE || B_RESTORE_HELD_BATTLE_ITEMS >= GEN_9)
            //TryRestoreHeldItems(); //HYDRA delete later if not needed.
...
static void ReturnFromBattleToOverworld(void)
{
    ...
    //HYDRA Carry the battle's final weather back to the overworld (persists until you enter an area
    // that forces its own weather). The intro already carried the overworld weather INTO the battle,
    // so an unchanged battle re-applies the same weather (no visible change); but if a move/ability
    // changed it -- or it ended (Rain Dance ran out) -- the overworld now matches. Skip link battles.
    if (!(gBattleTypeFlags & BATTLE_TYPE_LINK))
    {
        u32 owWeather;
        if (gBattleWeather & B_WEATHER_RAIN)
            owWeather = WEATHER_RAIN;
        else if (gBattleWeather & B_WEATHER_SUN)
            owWeather = WEATHER_DROUGHT;
        else if (gBattleWeather & B_WEATHER_SANDSTORM)
            owWeather = WEATHER_SANDSTORM;
        else if (gBattleWeather & B_WEATHER_HAIL) //HYDRA hail -> the new overworld hail weather (not snow)
            owWeather = WEATHER_HAIL;
        else if (gBattleWeather & B_WEATHER_SNOW)
            owWeather = WEATHER_SNOW;
        else
            owWeather = WEATHER_NONE;
        SetWeather(owWeather);
    }
