...
static void MasterBallOpenParticleAnimation(u8 taskId)
{
    ...
    for (j = 0; j < 2; j++)
    {
        for (i = 0; i < 4; i++) //HYDRA i < 8 to i < 4
        {
            spriteId = CreateSpriteUnchecked(&sBallParticles[ballId].spriteTemplate, x, y, subpriority);
            if (spriteId != MAX_SPRITES)
            {
...
{
    //HYDRA - wait for the real launch white-fade to finish (keeps the menu palette intact), then hand
    //        off WITHOUT starting the flash-back here. Starting it here re-activated the shared
    //        gPaletteFade and blocked the 2nd opponent's task -> its tint lingered (pink desync).
    if (!gPaletteFade.active)
...
static void Task_FadeMon_ToNormal_Step(u8 taskId)
{
    ...
    if (gTasks[taskId].tTimer == 0) //HYDRA - kick off the BG flash-back once, from Step
    {
        u32 selectedPalettes = (u16)gTasks[taskId].tPaletteLo | ((u16)gTasks[taskId].tPaletteHi << 16);
        BeginNormalPaletteFade(selectedPalettes, 0, 16, 0, RGB_WHITE);
    }
