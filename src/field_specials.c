...
bool8 IsNicknamePromptDisabled(void)
{
    return gSaveBlock2Ptr->optionsNicknamePromptOff;
}
...
//HYDRA HYPER PC screen frames. The decoration is two metatiles tall (screen half +
// base half) and each half has a floor-backed and a wall-backed variant, so all
// four have to be mapped individually.
static u16 HyperPC_GetSwappedMetatile(u16 current, bool8 turnOn)
{
    if (turnOn)
    {
        switch (current)
        {
        case METATILE_SecretBase_HyperTrainingPC_Top:      return METATILE_SecretBase_HyperTrainingPC_TopOn;
        case METATILE_SecretBase_HyperTrainingPC_TopWall:  return METATILE_SecretBase_HyperTrainingPC_TopWallOn;
        case METATILE_SecretBase_HyperTrainingPC_Base:     return METATILE_SecretBase_HyperTrainingPC_BaseOn;
        case METATILE_SecretBase_HyperTrainingPC_BaseWall: return METATILE_SecretBase_HyperTrainingPC_BaseWallOn;
        }
    }
    else
    {
        switch (current)
        {
        case METATILE_SecretBase_HyperTrainingPC_TopOn:      return METATILE_SecretBase_HyperTrainingPC_Top;
        case METATILE_SecretBase_HyperTrainingPC_TopWallOn:  return METATILE_SecretBase_HyperTrainingPC_TopWall;
        case METATILE_SecretBase_HyperTrainingPC_BaseOn:     return METATILE_SecretBase_HyperTrainingPC_Base;
        case METATILE_SecretBase_HyperTrainingPC_BaseWallOn: return METATILE_SecretBase_HyperTrainingPC_BaseWall;
        }
    }
    return current; // not one of ours, leave it alone
}

static void HyperPC_SetScreen(bool8 turnOn)
{
    s16 x, y;
    u8 i;

    // The base half is the tile the player is facing; the screen half sits above it.
    GetXYCoordsOneStepInFrontOfPlayer(&x, &y);

    for (i = 0; i < 2; i++, y--)
    {
        u16 current = MapGridGetMetatileIdAt(x, y);
        u16 swapped = HyperPC_GetSwappedMetatile(current, turnOn);

        if (swapped != current)
        {
            // MapGridSetMetatileIdAt takes the collision bits from its argument
            // (only elevation is preserved), so carry the existing collision over
            // or the solid base half would silently become walkable.
            MapGridSetMetatileIdAt(x, y,
                swapped | (MapGridGetCollisionAt(x, y) << MAPGRID_COLLISION_SHIFT));
        }
    }
...
void HyperPC_TurnScreenOn(void)
{
    HyperPC_SetScreen(TRUE);
}

void HyperPC_TurnScreenOff(void)
{
    HyperPC_SetScreen(FALSE);
}

//HYDRA HYPER PC: waits for the fade-to-black to finish before tearing down the
// overworld and entering the PC. Calling ChooseMonForHyperTraining() straight from
// the special made the storage screen build itself in full view over the overworld,
// with the wrong palettes loaded, before it finally faded. This mirrors
// Task_ChooseBoxMon in chooseboxmon.c, which is the working version of this flow.
static void Task_HyperPC_OpenBox(u8 taskId)
{
    if (!gPaletteFade.active)
    {
...
//HYDRA HYPER PC: opens the PC box screen with the cursor on the first party slot.
// Selecting a Pokemon offers HYPER TRAIN / NICKNAME / CANCEL. Box Pokemon are
// selectable too, and both actions return to the box afterwards.
void HyperPC_OpenHyperSummary(void)
{
    LockPlayerFieldControls();
    FadeScreen(FADE_TO_BLACK, 0);
    CreateTask(Task_HyperPC_OpenBox, 10);
}
