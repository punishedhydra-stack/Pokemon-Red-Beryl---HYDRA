...
#include "hydra_incubator.h" //HYDRA egg incubator sprite animation task
#include "hydra_radio.h" //HYDRA radio decoration playback
...
//HYDRA Remembers the tile in front of the player's own secret base entrance so Fly/Teleport can land there.
static bool8 sHydraDoorValid = FALSE;
static u16 sHydraDoorMapSec = 0;
static u16 sHydraDoorLocation = 0;
static u16 sHydraDoorX = 0;
static u16 sHydraDoorY = 0;

//HYDRA Commit the recorded entrance position into the save vars the region map reads
static void HydraCommitSecretBaseDoor(void)
{
    if (!sHydraDoorValid)
        return;

    VarSet(VAR_SECRET_BASE_DOOR_MAPSEC, sHydraDoorMapSec);
    VarSet(VAR_SECRET_BASE_DOOR_LOCATION, sHydraDoorLocation);
    VarSet(VAR_SECRET_BASE_DOOR_X, sHydraDoorX);
    VarSet(VAR_SECRET_BASE_DOOR_Y, sHydraDoorY);
    VarSet(VAR_SECRET_BASE_DOOR_SET, 1);
}

//HYDRA Record the tile the player is currently stood on as the base's front door.
//HYDRA Only valid out on the overworld route - never inside the base itself.
static void HydraCaptureSecretBaseDoorFromPlayer(void)
{
    if (CurMapIsSecretBase())
        return;

    sHydraDoorMapSec = gMapHeader.regionMapSectionId;
    sHydraDoorLocation = (((u8)gSaveBlock1Ptr->location.mapGroup) << 8) | ((u8)gSaveBlock1Ptr->location.mapNum);
    sHydraDoorX = gObjectEvents[gPlayerAvatar.objectEventId].currentCoords.x - MAP_OFFSET;
    sHydraDoorY = gObjectEvents[gPlayerAvatar.objectEventId].currentCoords.y - MAP_OFFSET;
    sHydraDoorValid = TRUE;
}
...
void SetPlayerSecretBase(void)
{
    ...
    //HYDRA record the front door of the base we just claimed (works whether this runs on the route or inside)
    HydraCaptureSecretBaseDoorFromPlayer();
    HydraCommitSecretBaseDoor();
}
...
{
    //HYDRA the player is still stood at the door on the route here - record it the moment they pick YES
    HydraCaptureSecretBaseDoorFromPlayer();
    HydraCommitSecretBaseDoor();
...
void InitSecretBaseDecorationSprites(void)
{
    ...
    for (i = 0; i < numDecorations; i++)
    {
        ...
        if (permission == DECORPERM_SPRITE)
        {
            ...
            if (MetatileBehavior_HoldsSmallDecoration(metatileBehavior) == TRUE
             ...
             || decorations[i] == DECOR_EGG_INCUBATOR //HYDRA incubator sits on the bare floor, not on furniture
             || decorations[i] == DECOR_RADIO //HYDRA radio also sits on the bare floor
             || (decorations[i] >= DECOR_RED_BED && decorations[i] <= DECOR_PINK_BED)) //HYDRA healing beds sit on the bare floor
            {
                ...
                //HYDRA Give the egg incubator its interaction script. Unlike the doll/cushion
                // TV-flag scripts (which only run in a friend's base), the incubator must be
                // usable in the player's own base, so this is outside the base-id check.
                if (decorations[i] == DECOR_EGG_INCUBATOR || decorations[i] == DECOR_RADIO
                 || (decorations[i] >= DECOR_RED_BED && decorations[i] <= DECOR_PINK_BED)) //HYDRA MISC sprite decorations need their interaction script in the player's own base too
                {
    ...
    HydraIncubator_StartAnimTask(); //HYDRA drive incubator sprite frames from egg state
    HydraRadio_PlayOnEnter(); //HYDRA start the chosen radio track in the player's own base
}
...
void SetCurSecretBaseIdFromPosition(const struct MapPosition *position, const struct MapEvents *events)
{
    ...
    for (i = 0; i < events->bgEventCount; i++)
    {
          ...
          && position->y == events->bgEvents[i].y + MAP_OFFSET)
        {
            ...
            //HYDRA remember this entrance's front door (one tile below the entrance metatile)
            sHydraDoorMapSec = gMapHeader.regionMapSectionId;
            sHydraDoorLocation = (((u8)gSaveBlock1Ptr->location.mapGroup) << 8) | ((u8)gSaveBlock1Ptr->location.mapNum);
            sHydraDoorX = events->bgEvents[i].x;
            sHydraDoorY = events->bgEvents[i].y + 1;
            sHydraDoorValid = TRUE;
            //HYDRA if this is already the player's own base, record it now (back-fills existing saves)
