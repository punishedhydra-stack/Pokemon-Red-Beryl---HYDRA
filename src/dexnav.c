...
//HYDRA
#define DN_SILHOUETTE_PAL_TAG 0x4442 // arbitrary unused sprite-palette tag

//HYDRA
static void LoadDexNavSilhouettePalette(void)
{
    u16 pal[16];
    u32 i;
    struct SpritePalette silhouettePal = {pal, DN_SILHOUETTE_PAL_TAG};
    pal[0] = RGB_BLACK;             // index 0 is always transparent
    for (i = 1; i < 16; i++)
        pal[i] = RGB(10, 10, 10);   // flat dark grey — tweak to taste
    LoadSpritePalette(&silhouettePal);
}

//HYDRA
static void CreateMonIconSilhouette(enum Species species, s16 x, s16 y)
{
    u8 spriteId = CreateMonIcon(species, SpriteCB_MonIcon, x, y, 0, 0xFFFFFFFF);
    if (spriteId != MAX_SPRITES)
        gSprites[spriteId].oam.paletteNum = IndexOfSpritePaletteTag(DN_SILHOUETTE_PAL_TAG);
}
...
static void TryDrawIconInSlot(enum Species species, s16 x, s16 y)
{
    if (species == SPECIES_NONE || species > NUM_SPECIES)
        ...
        CreateMonIconSilhouette(species, x, y); //HYDRA grey silhouette instead of question mark
    else
...
static void DrawSpeciesIcons(void)
{
    ...
    LoadDexNavSilhouettePalette(); //HYDRA
    for (i = 0; i < LAND_WILD_COUNT; i++)
    {
