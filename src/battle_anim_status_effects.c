...
void AnimTask_StatsChange(u8 taskId)
{
    ...
    bool16 multipleBattlers = FALSE;
    u16 animArg = gBattleSpritesDataPtr->animationData->animArg;

    if (animArg & STAT_ANIM_MULTIPLE_BATTLERS)
    {
        multipleBattlers = TRUE;
        animArg &= ~STAT_ANIM_MULTIPLE_BATTLERS;
    }
    ...
    gBattleAnimArgs[3] = multipleBattlers; // Whether it hits multiple battlers
