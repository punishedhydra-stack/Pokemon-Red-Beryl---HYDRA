...
#include "constants/follower_npc.h" //HYDRA
...
enum FlagsVarsDebugMenu
{
	...
	DEBUG_FLAGVAR_MENU_ITEM_TOGGLE_DEXNAV, //HYDRA
...
// *******************************
// Constants
#define FNPC_ALL_HYDRA 0 //HYDRA renamed from FNPC_ALL: collided with follower_npc.h's FNPC_ALL (FOLLOWER_NPC_FLAG_ALL)
...
static void DebugAction_FlagsVars_SwitchDexNav(u8 taskId); //HYDRA
...
static const struct DebugMenuOption sDebugMenu_Actions_Utilities[] =
{
	...
	{ COMPOUND_STRING("Sprite Counter"), 	Debug_ToggleSpriteCounter },
...
static const struct DebugMenuOption sDebugMenu_Actions_Flags[] =
{
	...
	[DEBUG_FLAGVAR_MENU_ITEM_TOGGLE_DEXNAV]        = { COMPOUND_STRING("Toggle {STR_VAR_1}DexNav"),          DebugAction_ToggleFlag, DebugAction_FlagsVars_SwitchDexNav }, //HYDRA
...
static u32 Debug_CheckToggleFlags(u8 id)
{
    ...
    case DEBUG_FLAGVAR_MENU_ITEM_TOGGLE_POKENAV:
        ...
        break;
	case DEBUG_FLAGVAR_MENU_ITEM_TOGGLE_DEXNAV: //HYDRA
        result = FlagGet(DN_FLAG_DEXNAV_GET);   //HYDRA
        break;                                  //HYDRA
...
static void DebugAction_Util_Weather_SelectId(u8 taskId)
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        if (gTasks[taskId].tInput <= 16 || gTasks[taskId].tInput >= 20)
        {
...
// *******************************
// Actions Sound
const u8 *const sSongNames[]; //HYDRA was static; reused by the radio decoration (src/hydra_radio.c)
...
static void DebugAction_CreateFollowerNPC(u8 taskId)
{
    ...
    CreateFollowerNPC(gfx, FNPC_ALL_HYDRA, Debug_Follower_NPC_Event_Script); //HYDRA renamed macro
    UnlockPlayerFieldControls();
}
...
// ===== SPRITE COUNTER IMPLEMENTATION =====

static bool8 sSpriteCounterActive = FALSE;
static u8 sSpriteCounterWindowId = 0;
static u8 sSpriteCounterTaskId = 0;

static const struct WindowTemplate sSpriteCounterWindowTemplate = {
    .bg = 0,
    .tilemapLeft = 22,   // Moved from 24 to 1 (left side)
    .tilemapTop = 12,    // Moved from 18 to 1 (top)
    .width = 7,
    .height = 4,
    .paletteNum = 15,
    .baseBlock = 0x1F0,
...
static void Task_UpdateLiveSpriteCounter(u8 taskId)
{
    u32 count = 0;
    u32 palettesUsed = 0;  // ADD THIS
    
    if (!sSpriteCounterActive) {
        if (sSpriteCounterWindowId != 0) {
            ClearStdWindowAndFrame(sSpriteCounterWindowId, TRUE);
            RemoveWindow(sSpriteCounterWindowId);
            sSpriteCounterWindowId = 0;
        }
        if (sSpriteCounterTaskId != 0) {
            DestroyTask(sSpriteCounterTaskId);
            sSpriteCounterTaskId = 0;
        }
        DestroyTask(taskId);
        return;
    }
    ...
    // ADD: Count palettes used (check if first color of each palette is non-zero)
    for (u32 i = 0; i < 16; i++) {
        if (gPlttBufferUnfaded[256 + (i * 16)] != 0)
            palettesUsed++;
    }
    
    if (sSpriteCounterWindowId != 0) {
        FillWindowPixelBuffer(sSpriteCounterWindowId, PIXEL_FILL(1));
        
        // Line 1: Sprites count
        ConvertIntToDecimalStringN(gStringVar1, count, STR_CONV_MODE_LEFT_ALIGN, 3);
        StringCopy(gStringVar2, COMPOUND_STRING("Spr:"));
        StringAppend(gStringVar2, gStringVar1);
        AddTextPrinterParameterized(sSpriteCounterWindowId, FONT_SMALL, 
            gStringVar2, 0, 0, 0, NULL);
        
        // Line 2: Palette count (e.g., "Pal:12/16")
        ConvertIntToDecimalStringN(gStringVar1, palettesUsed, STR_CONV_MODE_LEFT_ALIGN, 2);
        StringCopy(gStringVar2, COMPOUND_STRING("Pal:"));
        StringAppend(gStringVar2, gStringVar1);
        StringAppend(gStringVar2, COMPOUND_STRING("/16"));
        AddTextPrinterParameterized(sSpriteCounterWindowId, FONT_SMALL, 
            gStringVar2, 0, 12, 0, NULL);  // CHANGED y from 8 to 12
        
        CopyWindowToVram(sSpriteCounterWindowId, COPYWIN_FULL);
    }
}

void Debug_ToggleSpriteCounter(void)
{
    // Check if task was destroyed externally (battle, etc.) and reset if so
    if (sSpriteCounterTaskId != 0 && FindTaskIdByFunc(Task_UpdateLiveSpriteCounter) == TASK_NONE)
    {
        sSpriteCounterTaskId = 0;
        sSpriteCounterWindowId = 0;
        sSpriteCounterActive = FALSE;
    }
    
    sSpriteCounterActive = !sSpriteCounterActive;
    
    if (sSpriteCounterActive) {
        // Turn ON
        if (sSpriteCounterWindowId == 0) {
            sSpriteCounterWindowId = AddWindow(&sSpriteCounterWindowTemplate);
            DrawStdWindowFrame(sSpriteCounterWindowId, FALSE);
            CopyWindowToVram(sSpriteCounterWindowId, COPYWIN_FULL);
        }
        // FIXED: Check if task actually exists, not just if ID is non-zero
        if (sSpriteCounterTaskId == 0 || FindTaskIdByFunc(Task_UpdateLiveSpriteCounter) == TASK_NONE) {
            sSpriteCounterTaskId = CreateTask(Task_UpdateLiveSpriteCounter, 0);
        }
        PlaySE(SE_PC_ON);
    } else {
        PlaySE(SE_PC_OFF);
...
// For menu display - shows current state
bool8 Debug_IsSpriteCounterActive(void)
{
    return sSpriteCounterActive;
}