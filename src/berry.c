...
#include "item.h"
#include "item_menu.h"
#include "list_menu.h" // //HYDRA
#include "main.h"
#include "malloc.h" // //HYDRA
#include "script_menu.h" // //HYDRA
#include "random.h"
...
#include "constants/script_menu.h" // //HYDRA
...
// //HYDRA Which bit of tree->watered corresponds to a growth stage.
// Mirrors the stage -> bit mapping in ObjectEventInteractionWaterBerryTree (berry.c:2396).
// Returns 0 for stages that can't be watered at all.
static u8 GetWateredBitForStage(u8 stage)
{
    switch (stage)
    {
    case BERRY_STAGE_PLANTED:
...
{
    struct BerryTree *tree = GetBerryTreeInfo(GetObjectEventBerryTreeId(gSelectedObjectEvent));
    u8 wateredBit = GetWateredBitForStage(tree->stage);

    // //HYDRA each growth stage can only be watered once
    if (wateredBit == 0 || (tree->watered & wateredBit))
        return FALSE;

    if (!OW_BERRY_MOISTURE || OW_BERRY_ALWAYS_WATERABLE)
        return TRUE;
    return (tree->moistureLevel == 0);
}

// //HYDRA TRUE if pressing A on the selected berry tree would actually do something.
// Called from TryStartInteractionScript so a growing tree you can't water is fully
// inert - no interaction sound, no script.
bool8 ShouldBerryTreeInteractionRun(void)
{
    u8 treeId = GetObjectEventBerryTreeId(gSelectedObjectEvent);
    u8 stage = GetStageByBerryTreeId(treeId);

    // Empty soil (plant/mulch) and fully grown (pick) always have something to do
    if (stage == BERRY_STAGE_NO_BERRY || stage == BERRY_STAGE_BERRIES)
        return TRUE;

    // Preserve the sparkle "!" interaction
    if (IsBerryTreeSparkling(gSpecialVar_LastTalked, gSaveBlock1Ptr->location.mapNum, gSaveBlock1Ptr->location.mapGroup))
        return TRUE;

    // Growing: only interact if there's a weed, pests, or it can actually be watered
    if (OW_BERRY_WEEDS && gSaveBlock1Ptr->berryTrees[treeId].weeds)
        return TRUE;
    if (OW_BERRY_PESTS && gSaveBlock1Ptr->berryTrees[treeId].pests)
        return TRUE;

    return (CheckBagHasItem(ITEM_WAILMER_PAIL, 1) && CanWaterBerryPlot());
}
...
// //HYDRA -------- Berry planting menu --------
// Start-menu style list of the BERRIES pocket, shown over the overworld instead of
// opening the full-screen bag. Row ids are the ROW INDEX, never item ids, so they
// can't collide with MULTI_B_PRESSED; SetBerryPlantSelection maps the index back.

static const u8 sText_BerryMenuQuantity[] = _("  x");
static const u8 sText_BerryMenuCancel[] = _("CANCEL");

static EWRAM_DATA enum Item sBerryMenuItemIds[BAG_BERRIES_COUNT] = {0};
static EWRAM_DATA u8 sBerryMenuCount = 0;

void BuildBerryPlantMenu(void)
{
    u32 i;
    u8 *name;
    ...
    {
        enum Item itemId = GetBagItemId(POCKET_BERRIES, i);
        ...
        // Same allocation pattern as ScrCmd_dynmultipush (scrcmd.c:1919)
        name = Alloc(100);
        StringCopy(name, GetItemName(itemId));
        StringAppend(name, sText_BerryMenuQuantity);
        ConvertIntToDecimalStringN(numBuffer, quantity, STR_CONV_MODE_LEFT_ALIGN, 3);
        StringAppend(name, numBuffer);

        menuItem.name = name;
        menuItem.id = sBerryMenuCount;
        MultichoiceDynamic_PushElement(menuItem);

        sBerryMenuItemIds[sBerryMenuCount] = itemId;
        sBerryMenuCount++;
    }

    // CANCEL is always the last row; its id is one past the final berry
    name = Alloc(100);
    StringCopy(name, sText_BerryMenuCancel);
    menuItem.name = name;
    menuItem.id = sBerryMenuCount;
    MultichoiceDynamic_PushElement(menuItem);
}

void SetBerryPlantSelection(void)
{
    u16 selection = gSpecialVar_Result;

    // B press or the CANCEL row -> nothing selected
    if (selection == MULTI_B_PRESSED || selection >= sBerryMenuCount)
        gSpecialVar_ItemId = ITEM_NONE;
    else
        gSpecialVar_ItemId = sBerryMenuItemIds[selection];
}
...
void ObjectEventInteractionPickBerryTree(void)
{
    ...
    // Add this line - store the count in gSpecialVar_Result for the popup
    gSpecialVar_Result = GetBerryCountByBerryTreeId(id);
