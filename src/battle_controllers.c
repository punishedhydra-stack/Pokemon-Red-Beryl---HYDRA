...
static bool8 ShouldDoSlideInAnim(enum BattlerId battler)
{
    ...
    // BATTLE_TYPE_DOUBLE removed so trainer/wild doubles can slide in.
    if (gBattleTypeFlags & (
        BATTLE_TYPE_LINK | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_FIRST_BATTLE |
    ...
    // Right-side lead pairs with the second follower; left with the first.
    if (GetBattlerPosition(battler) == B_POSITION_PLAYER_RIGHT)
    {
        followerObj = GetFollower2Object();
...
static u16 sAnimSpeedAccumulator = 0; //HYDRA - carries fractional anim ticks across frames
...
static void Controller_DoMoveAnimation(enum BattlerId battler)
{
    ...
    case 1:
        if (!gBattleSpritesDataPtr->healthBoxesData[battler].specialAnimActive)
        {
            ...
            DoMoveAnim(move);
            sAnimSpeedAccumulator = 0; //HYDRA - start this animation with a clean fractional accumulator
    ...
    {
        u32 i, ticks;
        u32 pct = (B_ANIM_SPEED_PERCENT < 100) ? 100 : B_ANIM_SPEED_PERCENT; //HYDRA - floor at 1.0x
        sAnimSpeedAccumulator += pct;               //HYDRA - accumulate fractional speed
        ticks = sAnimSpeedAccumulator / 100;        //HYDRA - whole anim ticks this frame (>= 1)
        sAnimSpeedAccumulator -= ticks * 100;       //HYDRA - keep the leftover fraction
        for (i = 1; i < ticks && gAnimScriptActive; i++) //HYDRA - natural frame gives 1 tick; run the rest
        {
            gAnimScriptCallback(); //HYDRA
            AnimateSprites();      //HYDRA
...
void BtlController_HandlePrintString(enum BattlerId battler)
{
    ...
    if (*stringId == STRINGID_PLAYERDEFEATEDTRAINER1)
    {
        enum BattlerId battlerLeft = GetBattlerAtPosition(B_POSITION_PLAYER_LEFT);
        enum BattlerId battlerRight = GetBattlerAtPosition(B_POSITION_PLAYER_RIGHT);
        BattleAnimateBackSprite(&gSprites[gBattlerSpriteIds[battlerLeft]], gBattleMons[battlerLeft].species);
        if (IsDoubleBattle() && IsBattlerAlive(battlerRight))
            BattleAnimateBackSprite(&gSprites[gBattlerSpriteIds[battlerRight]], gBattleMons[battlerRight].species);
    }
...
#define tDeferredPartner    data[6] // player side: partner send-out is held back
#define tPartnerTimeout     data[7]

// The player's pair are sent out one at a time so their entrance animations and cries
// don't overlap -- starting both on the same frame makes them slide in together and fire
// both cries simultaneously. The opponent is left alone; it uses the normal ball send-out
// and already sounds correct.
//
// The partner follows a short fixed delay later, so the two slide-ins overlap and read as
// one quick one-two rather than two separate entrances. The cries are kept from
// overlapping separately, in SpriteCB_PlayerMonSlideIn (battle_main.c), which waits for
// any playing cry before handing off -- so raising this only spaces the slides out, it
// can never cause two cries at once.
#define SENDOUT_STAGGER_FRAMES 12
...
static void Task_StartSendOutAnim(u8 taskId)
{
    ...
    // Second half of the staggered player send-out: the partner is released a short delay
    // after the first mon, so their slide-ins overlap slightly in party order.
    if (gTasks[taskId].tDeferredPartner)
    {
        if (gTasks[taskId].tPartnerTimeout < SENDOUT_STAGGER_FRAMES)
        {
            gTasks[taskId].tPartnerTimeout++;
            return;
        }
        ...
        StartSendOutAnim(battlerPartner, FALSE, FALSE, ShouldDoSlideInAnim(battlerPartner));
    ...
    else
    {
        if (TwoMonsAtSendOut(battler))
        {
            ...
            // Hold the player's partner back until this mon has finished, so only one
            // animation + cry runs at a time. The opponent keeps its original behaviour.
            if (IsOnPlayerSide(battler))
            {
                gTasks[taskId].tDeferredPartner = TRUE;
                gTasks[taskId].tPartnerTimeout = 0;
                return;
            }
...
#undef tDeferredPartner
#undef tPartnerTimeout
