...
#include "hydra_hotbar.h" //HYDRA fly-map cancel routing back to the hotbar
...
//HYDRA TRUE if this map section is the route holding the player's own secret base
static bool32 HydraIsPlayerSecretBaseMapSec(mapsec_u16_t mapSecId)
{
    return gSaveBlock1Ptr->secretBases[0].secretBaseId != 0
        && VarGet(VAR_SECRET_BASE_DOOR_SET) != 0
        && mapSecId != MAPSEC_NONE
        && mapSecId == VarGet(VAR_SECRET_BASE_DOOR_MAPSEC);
}

static u8 GetMapsecType(mapsec_u16_t mapSecId)
{
    //HYDRA the player's own secret base is selectable, drawn with the red outline icon
    if (HydraIsPlayerSecretBaseMapSec(mapSecId))
...
static void TryCreateRedOutlineFlyDestIcons(void)
{
    ...
    //HYDRA red outline icon over the player's own secret base
    if (HydraIsPlayerSecretBaseMapSec(VarGet(VAR_SECRET_BASE_DOOR_MAPSEC)))
    {
        mapSecId = VarGet(VAR_SECRET_BASE_DOOR_MAPSEC);
        GetMapSecDimensions(mapSecId, &x, &y, &width, &height);
        x = (x + MAPCURSOR_X_MIN) * 8;
    ...
    case 1:
        if (!UpdatePaletteFade())
        {
            ...
            if (sFlyMap->choseFlyLocation)
            {
                ...
                gHydraFlyMapFromHotbar = FALSE; //HYDRA flew to a destination; not returning to the hotbar
                SetFlyDestination(tempRegionMap);
                ReturnToFieldFromFlyMapSelect();
            }
            else if (gHydraFlyMapFromHotbar) //HYDRA fly map was opened from the hotbar: cancel -> overworld + hotbar
            {
                gHydraFlyMapFromHotbar = FALSE;
                SetMainCallback2(CB2_HydraHotbar_ReturnToField);
            }
            else
            {
...
void SetFlyDestination(struct RegionMap* regionMap)
{
    ...
    //HYDRA land at the front door of the player's own secret base instead of a heal location
    if (HydraIsPlayerSecretBaseMapSec(regionMap->mapSecId))
    {
        u16 loc = VarGet(VAR_SECRET_BASE_DOOR_LOCATION);

        SetWarpDestination((s8)(loc >> 8), (s8)(loc & 0xFF), WARP_ID_NONE,
                           VarGet(VAR_SECRET_BASE_DOOR_X), VarGet(VAR_SECRET_BASE_DOOR_Y));
        return;
    }
