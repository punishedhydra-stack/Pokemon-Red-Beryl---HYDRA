...
#include "battle.h"
#include "m4a.h"
#include "battle_anim.h" //HYDRA - for gAnimScriptActive and gBattleAnimSpeedPercent
#include "main.h"
#include "overworld.h"
#include "pokemon.h"
...
static bool8 sBerryFanfareActive = FALSE; //HYDRA
...
//HYDRA - TRUE while the berry pickup fanfare is still playing.
static bool8 IsBerryFanfarePlaying(void) //HYDRA
{
    return (sBerryFanfareActive && !IsFanfareTaskInactive());
}
...
void PlayFanfare(u16 songNum)
{
    ...
    //HYDRA - the berry pickup fanfare is allowed to finish. Another berry pickup
    //HYDRA - restarts it; any other fanfare is dropped while it is still playing.
    if (songNum == MUS_OBTAIN_BERRY)
        sBerryFanfareActive = TRUE;
    else if (IsBerryFanfarePlaying())
        return;
...
static void Task_Fanfare(u8 taskId)
{
    if (gTestRunnerHeadless)
    {
        DestroyTask(taskId);
        sFanfareCounter = 0;
        sBerryFanfareActive = FALSE; //HYDRA
        return;
    }
...
{
    sBerryFanfareActive = FALSE; //HYDRA - a real BGM change (battle, map music) cuts the fanfare
    if (gDisableMusic)
        songNum = 0;
    if (songNum == MUS_NONE)
        songNum = 0;
...
//HYDRA - convert a speed percent (100 = 1.0x) to an m4a pitch bend (256 units = 1 semitone),
//HYDRA - so a sound effect plays percent/100 faster. pitch = 3072 * log2(percent/100).
//HYDRA - verified accurate to < 0.03% across 100-400.
static s16 SpeedPercentToPitch(u16 percent) //HYDRA
{
    u32 x;
    s32 intPart, frac, i, log2Q16, pitch;
    if (percent <= 100)
        return 0;
    x = ((u32)percent << 16) / 100;                   //HYDRA - ratio in Q16 (>= 65536)
    intPart = 0;
    while (x >= 131072u) { x >>= 1; intPart++; }       //HYDRA - reduce to [1,2), count octaves
    frac = 0;
    for (i = 0; i < 16; i++)                            //HYDRA - 16-bit fractional log2 by squaring
    {
        x = (u32)(((u64)x * x) >> 16);
        frac <<= 1;
        if (x >= 131072u) { x >>= 1; frac |= 1; }
    }
    log2Q16 = (intPart << 16) + frac;
    pitch = (s32)(((s64)log2Q16 * 3072) >> 16);        //HYDRA - 3072 units = one octave
    return (s16)pitch;
}
//HYDRA - while an attack animation is playing at boosted speed, pitch its SE up to match.
static void ApplyBattleAnimSpeedToSE(struct MusicPlayerInfo *mplay) //HYDRA
{
    if (gAnimScriptActive && B_ANIM_SPEED_PITCH_PERCENT > 100)
        m4aMPlayPitchControl(mplay, TRACKS_ALL, SpeedPercentToPitch(B_ANIM_SPEED_PITCH_PERCENT));
}
void PlaySE(u16 songNum)
{
    if (IsBerryFanfarePlaying()) //HYDRA - don't let menu/interaction SEs step on the berry fanfare
        return;

    if (gDisableMapMusicChangeOnMapLoad == MUSIC_DISABLE_OFF)
    {
        m4aSongNumStart(songNum);
        ApplyBattleAnimSpeedToSE(&gMPlayInfo_SE1); //HYDRA - SE could route to either channel
        ApplyBattleAnimSpeedToSE(&gMPlayInfo_SE2); //HYDRA
    }
}
...
void PlaySE12WithPanning(u16 songNum, s8 pan)
{
    ...
    ApplyBattleAnimSpeedToSE(&gMPlayInfo_SE1); //HYDRA
    ApplyBattleAnimSpeedToSE(&gMPlayInfo_SE2); //HYDRA
}
