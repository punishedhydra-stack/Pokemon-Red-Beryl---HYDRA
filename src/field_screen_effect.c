...
#include "global.h"
#include "hydra_party_panel.h" //HYDRA re-grey after the return fade settles
...
//HYDRA saved time-of-day blend while the start-menu return fade suppresses it (see below).
static struct TimeBlendSettings sPanelReturnSavedTimeBlend;
static bool8 sPanelReturnTODSuppressed = FALSE;

static void Task_WaitForFadeShowStartMenu(u8 taskId)
{
    //HYDRA re-grey EVERY frame while the return fade runs, not only when it finishes. The tall-
    // grass field effect respawns and loads its COLOUR palette partway through the fade, so a
    // once-at-the-end reapply left the grass green for a few frames of the reveal (SS3). Re-greying
    // each frame (faded + unfaded fade source) keeps the grass grey the whole way in (SS4).
    HydraPartyPanel_ReapplyGrayscale();
    if (WaitForWeatherFadeIn() == TRUE)
    {
        //HYDRA restore the time-of-day blend now that the grey reveal is done (see ReturnToField-
        // OpenStartMenu). Kept suppressed until the fade fully settles so no TOD tint bleeds in.
        if (sPanelReturnTODSuppressed)
        {
            gTimeBlend = sPanelReturnSavedTimeBlend;
            sPanelReturnTODSuppressed = FALSE;
        }
        HydraPartyPanel_ReapplyGrayscale(); //HYDRA final re-grey once the fade has fully settled
        DestroyTask(taskId);
...
{
    FadeInFromBlack();
    //HYDRA suppress the time-of-day tint during the grey reveal (mirror the hotbar's reopen). Every
    // frame UpdatePaletteFade mixes gTimeBlend into the faded buffer, which re-tinted both the BG and
    // the (non-immune) grayscale sprite palette while the screen faded back in - so the reveal showed
    // TOD-tinted grey instead of pure grey (SS4 BG tint, SS5 sprites stay tinted). Zeroing the blend
    // coeffs makes that mix a passthrough. Restored in Task_WaitForFadeShowStartMenu once the fade
    // settles. Done AFTER FadeInFromBlack because FadeScreen recomputes gTimeBlend.
    if (HydraPartyPanel_IsOpen())
    {
        sPanelReturnSavedTimeBlend = gTimeBlend;
        gTimeBlend.startBlend.coeff = 0;
        gTimeBlend.startBlend.isTint = 0;
        gTimeBlend.endBlend.coeff = 0;
        gTimeBlend.endBlend.isTint = 0;
        sPanelReturnTODSuppressed = TRUE;
    }
...
void Task_DoDoorWarp(u8 taskId)
{
    ...
    struct ObjectEvent *followerObject2 = GetFollower2Object();
    ...
    case DOORWARP_OPEN_DOOR:
        ...
        if (followerObject2)
        {
            // Put follower 2 into its pokeball too
            ClearObjectEventMovement(followerObject2, &gSprites[followerObject2->spriteId]);
            ObjectEventSetHeldMovement(followerObject2, MOVEMENT_ACTION_ENTER_POKEBALL);
        }
