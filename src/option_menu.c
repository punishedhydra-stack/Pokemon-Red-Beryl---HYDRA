...
#define tNicknamePromptOff data[3]
...
#define YPOS_NICKNAME     (MENUITEM_NICKNAME * 16)
...
static u8 NicknamePrompt_ProcessInput(u8 selection);
static void NicknamePrompt_DrawChoices(u8 selection);
...
static const u8 gText_NicknamePromptOn[]   = _("{COLOR GREEN}{SHADOW LIGHT_GREEN}ON");
static const u8 gText_NicknamePromptOff[]  = _("{COLOR GREEN}{SHADOW LIGHT_GREEN}OFF");
...
static const u8 *const sOptionMenuItemsNames[MENUITEM_COUNT] =
{
    ...
    [MENUITEM_NICKNAME]    = COMPOUND_STRING("NICKNAME"),
...
void CB2_InitOptionMenu(void)
{
    ...
    case 10:
    {
        ...
        gTasks[taskId].tNicknamePromptOff = gSaveBlock2Ptr->optionsNicknamePromptOff;
        ...
        NicknamePrompt_DrawChoices(gTasks[taskId].tNicknamePromptOff);
...
static void Task_OptionMenuProcessInput(u8 taskId)
{
    ...
    else
    {
        ...
        case MENUITEM_NICKNAME:
            previousOption = gTasks[taskId].tNicknamePromptOff;
            gTasks[taskId].tNicknamePromptOff = NicknamePrompt_ProcessInput(gTasks[taskId].tNicknamePromptOff);

            if (previousOption != gTasks[taskId].tNicknamePromptOff)
                NicknamePrompt_DrawChoices(gTasks[taskId].tNicknamePromptOff);
            break;
        case MENUITEM_SOUND:
...
static void Task_OptionMenuSave(u8 taskId)
{
    ...
    gSaveBlock2Ptr->optionsNicknamePromptOff = gTasks[taskId].tNicknamePromptOff;
...
static void NicknamePrompt_DrawChoices(u8 selection)
{
    ...
    DrawOptionMenuChoice(gText_NicknamePromptOn, 104, YPOS_NICKNAME, styles[0]);
    DrawOptionMenuChoice(gText_NicknamePromptOff, GetStringRightAlignXOffset(FONT_NORMAL, gText_NicknamePromptOff, 198), YPOS_NICKNAME, styles[1]);
}
