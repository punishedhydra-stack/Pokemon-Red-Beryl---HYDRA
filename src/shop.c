...
//HYDRA A shop item is "sold out" once the player owns the maximum allowed. Normal item marts
// reuse the vanilla one-per-important-item rule; the decoration mart caps the HYPER PC at 1 and
// the egg incubator at 6 (counting both inventory and placed copies).
static bool32 IsShopItemSoldOut(u32 itemId)
{
    if (sMartInfo.martType == MART_TYPE_NORMAL)
        return GetItemImportance(itemId) && (CheckBagHasItem(itemId, 1) || CheckPCHasItem(itemId, 1));
    if (itemId == DECOR_HYPER_TRAINING_PC)
        return CountOwnedDecoration(itemId) >= 1;
    if (itemId == DECOR_EGG_INCUBATOR)
        return CountOwnedDecoration(itemId) >= 6;
    if (itemId == DECOR_RADIO) //HYDRA limit 1 radio
        return CountOwnedDecoration(itemId) >= 1;
    return FALSE;
}
...
static void BuyMenuPrintPriceInList(u8 windowId, u32 itemId, u8 y)
{
    ...
    if (itemId != LIST_CANCEL)
    {
        ...
        if (IsShopItemSoldOut(itemId)) //HYDRA decoration caps + vanilla item rule
...
static void BuyMenuTryMakePurchase(u8 taskId)
{
    ...
    if (sMartInfo.martType == MART_TYPE_NORMAL)
    {
        if (AddBagItem(tItemId, tItemCount) == TRUE)
        {
            ...
            if (IsVialItem(tItemId)) //HYDRA - one evolving vial: swap out old tier + fill charges
                OnVialAcquired(tItemId);
