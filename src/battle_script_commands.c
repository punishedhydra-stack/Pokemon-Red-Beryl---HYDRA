...
extern u16 gStolenItemToAnnounce;
...
static void Cmd_tryrestorehelditems(void);
static void Cmd_preparestolenitemmessage(void);
...
void (*const gBattleScriptingCommandsTable[])(void) =
{
	...
	[B_SCR_OP_TRYRESTOREHELDITEMS]					 = Cmd_tryrestorehelditems,
	[B_SCR_OP_PREPARESTOLENITEMMESSAGE] 			 = Cmd_preparestolenitemmessage,
...
void StealTargetItem(enum BattlerId battlerStealer, enum BattlerId itemBattler)
{
    ...
    else
    {
		...
		if (gStolenItemCount < PARTY_SIZE) {
			gStolenItems[gStolenItemCount] = gLastUsedItem;
			gStolenItemCount++;
		}
...
void SetMoveEffect(enum BattlerId battlerAtk, enum BattlerId effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    case MOVE_EFFECT_PAYDAY:
        ...
        if (IsOnPlayerSide(battlerAtk) && gSpecialStatuses[battlerAtk].parentalBondState != PARENTAL_BOND_2ND_HIT)
        {
            ...
            gPaydayMoney += (gBattleMons[battlerAtk].level * 150);
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gParties[B_TRAINER_PLAYER][*expMonId]))
                {
                    ...
                    {
                        // PrepareStringBattle(STRINGID_PKMNGAINEDEXP, gBattleStruct->expGetterBattlerId); //HYDRA
                    }
                    else if (IsGen6ExpShareEnabled() && !gBattleStruct->teamGotExpMsgPrinted) // Print 'the rest of your team got exp' message once, when all of the sent-in mons were given experience
                    {
                        ...
                        //PrepareStringBattle(STRINGID_TEAMGAINEDEXP, gBattleStruct->expGetterBattlerId); //HYDRA
...
void BS_TryRestoreHeldItems(void)
{
    TryRestoreHeldItems();
    gBattlescriptCurrInstr += 1;  // Commands are 1 byte
}

void BS_PrepareStolenItemMessage(void)
{
    if (gStolenItemToAnnounce != ITEM_NONE) {
        PREPARE_ITEM_BUFFER(gBattleTextBuff1, gStolenItemToAnnounce);
        gStolenItemToAnnounce = ITEM_NONE;
    } else {
...
static void Cmd_useitemonopponent(void) //HYDRA
{
    CMD_ARGS();

    // Check if this is a trainer battle (not wild battle)
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
        // Print message and don't use item
        PrepareStringBattle(STRINGID_NOITEMSINTRAINERBATTLE, gBattlerAttacker);
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
    else
    {
        // Normal item usage for wild battles
...
static void Cmd_trygivecaughtmonnick(void)
{
    ...
    // If the in-game "Nickname" option is OFF, skip the naming screen entirely
    // and keep the caught mon's default (species) name. Mirrors the "No" exit (case 4).
...
void BS_JumpIfNicknamePromptOff(void)
{
