#include "global.h"
#include "script.h"
#include "hydra_incubator.h" //HYDRA re-arm egg incubator animation on field resume
#include "hydra_radio.h" //HYDRA re-apply the chosen radio track on field resume
...
void RunOnResumeMapScript(void)
{
    ...
    HydraIncubator_StartAnimTask(); //HYDRA re-arm the incubator animation after any menu/cutscene
    HydraRadio_PlayOnEnter(); //HYDRA (re)apply the chosen radio track in the player's base
}

void RunOnReturnToFieldMapScript(void)
{
    MapHeaderRunScriptType(MAP_SCRIPT_ON_RETURN_TO_FIELD);
    HydraIncubator_StartAnimTask(); //HYDRA re-arm the incubator animation after returning to the field
    HydraRadio_PlayOnEnter(); //HYDRA (re)apply the chosen radio track in the player's base
}
