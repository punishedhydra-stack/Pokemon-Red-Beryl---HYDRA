...
//HYDRA ------------------- Tiered trainer AI -------------------
// Every normal trainer is bumped to HYDRA_AI_HARD; bosses additionally get
// HYDRA_AI_SUPER_HARD. Edit these two sets to tune difficulty.
//
// HARD (all trainers): plays competently and prioritises KOs + smart switching,
// but only knows your STAB moves (ASSUME_STAB), so it is tough without feeling
// like it is reading your mind.
#define HYDRA_AI_HARD  ( AI_FLAG_CHECK_BAD_MOVE           \
...
// SUPER HARD (gym leaders, Elite Four, Champion, Magma/Aqua admins & bosses):
// VGC Open-Team-Sheet knowledge plus prediction.
// AI_FLAG_OMNISCIENT is exactly the open team sheet: Ai_InitPartyStruct reveals, for
// ALL 6 of your party mons, their species/form, held item, ability, and all 4 moves
// from turn one (see the isOmniscient block in that function). AI_FLAG_PREDICTION
// then has it predict your switches, incoming mons, and moves off that full picture.
// (AI_FLAG_KNOW_OPPONENT_PARTY would only reveal species, which OMNISCIENT already does.)
#define HYDRA_AI_SUPER_HARD ( HYDRA_AI_HARD  \
                            | AI_FLAG_OMNISCIENT \
                            | AI_FLAG_PREDICTION \
                            | AI_FLAG_SMART_TERA )

// TRUE for the "boss" trainer classes that should get the super-hard AI.
static bool32 HydraIsBossTrainer(u16 trainerId)
{
    switch (GetTrainerClassFromId(trainerId))
    {
    case TRAINER_CLASS_LEADER:        // gym leaders
    case TRAINER_CLASS_ELITE_FOUR:
    case TRAINER_CLASS_CHAMPION:
    case TRAINER_CLASS_AQUA_ADMIN:
    case TRAINER_CLASS_AQUA_LEADER:   // Archie
    case TRAINER_CLASS_MAGMA_ADMIN:
    case TRAINER_CLASS_MAGMA_LEADER:  // Maxie
    case TRAINER_CLASS_LEADER_FRLG:   // included for safety; unused in an Emerald build
...
static u64 GetAiFlags(u16 trainerId, enum BattlerId battler)
{
    ...
    else
    {
        ...
        {
            //HYDRA Tiered difficulty. Keep the trainer's own flags (Ace Pokemon,
            // etc.) and OR in a strong baseline for everyone, plus an extra set for
            // bosses. Only affects normal trainer battles -- frontier/factory/wild
            // are handled by the branches above and are untouched.
            flags = GetTrainerAIFlagsFromId(trainerId) | HYDRA_AI_HARD;
            if (HydraIsBossTrainer(trainerId))
                flags |= HYDRA_AI_SUPER_HARD;
        }
    }
...
static u32 ChooseMoveOrAction_Singles(enum BattlerId battler)
{
    ...
    // Sanity check before random selection
	if (numOfBestMoves == 0)
		numOfBestMoves = 1;
	if (numOfBestMoves > MAX_MON_MOVES)
		numOfBestMoves = MAX_MON_MOVES;
