...
static bool32 WillStatChangeAffectBattler(struct BattleCalcValues *cv, enum BattlerId battler, bool32 goesDown);

// TRUE once a spread stat change has played its single both-battlers animation,
// so the remaining per-target stat animations in the sequence are skipped.
static bool32 sStatChangeAnimsBatched = FALSE;

// TRUE once a spread stat drop has shown its single combined message, so the
// remaining affected mons run their reactions silently (no duplicate text).
static bool32 sCombinedStatMsgPlayed = FALSE;
...
static enum StatChangeResult DecreaseStat(struct BattleCalcValues *cv, struct StatChange *st)
{
    ...
    if (!st->onlyChecking)
    {
        ...
        // Spread stat drop (e.g. Leer on both foes): show one combined message for the
        // whole side, but still run each mon's Defiant/Adrenaline reactions.
        if (WillStatChangeAffectBattler(cv, BATTLE_PARTNER(cv->battlerDef), TRUE))
        {
            if (sCombinedStatMsgPlayed)
            {
                st->script = BattleScript_DecreaseStatChangeSilent;
            }
            else
            {
                sCombinedStatMsgPlayed = TRUE;
                gBattleCommunication[MULTISTRING_CHOOSER] = GetBattlerSide(cv->battlerDef);
                st->script = BattleScript_DecreaseStatChangeMessageMultiple;
            }
        }
        else
        {
...
// HYDRA - For a damaging spread move whose stat change is a per-target SECONDARY effect
// (Icy Wind, Snarl, Struggle Bug, Electroweb, Breaking Swipe, Glaciate, Bulldoze), each foe is
// resolved in its own move-script pass, so the partner foe's stat queue is empty when the first
// foe animates. The queue-based WillStatChangeAffectBattler can't see it. This detects from move
// data whether the partner foe will receive the SAME guaranteed stat change, so both animate at
// once. Chance-based drops (Muddy Water, Springtide Storm) are intentionally left per-target,
// since each foe rolls independently.
static bool32 WillSpreadSecondaryStatHitPartner(struct BattleCalcValues *cv, struct StatChange *st, bool32 goesDown)
{
    enum BattlerId partner = BATTLE_PARTNER(cv->battlerDef);
    ...
    if (partner >= gBattlersCount || !IsBattlerAlive(partner))
        return FALSE;
    if (partner == gBattlerAttacker || IsBattlerAlly(partner, gBattlerAttacker))
        return FALSE; // partner must be a foe of the attacker
    if (gBattleStruct->moveResultFlags[partner] & MOVE_RESULT_NO_EFFECT)
        return FALSE; // partner missed / immune / protected
    if (goesDown && CompareStat(partner, st->stat, MIN_STAT_STAGE, CMP_EQUAL, ABILITY_NONE))
        return FALSE;
    if (!goesDown && CompareStat(partner, st->stat, MAX_STAT_STAGE, CMP_EQUAL, ABILITY_NONE))
        return FALSE;

    for (u32 i = 0; i < GetMoveAdditionalEffectCount(gCurrentMove); i++)
    {
        const struct AdditionalEffect *fx = GetMoveAdditionalEffectById(gCurrentMove, i);
        enum MoveEffect want = goesDown ? MOVE_EFFECT_STAT_MINUS : MOVE_EFFECT_STAT_PLUS;

        if (fx->moveEffect != want)
            continue;
        if (fx->self || fx->onSide)
            continue; // self-buffs and onSide moves already batch correctly
        if (fx->chance != 0 && fx->chance < 100)
            continue; // chance-based: each foe rolls independently
        if (GetStatStage(st->stat, fx) == 0)
            continue; // this effect doesn't touch the stat currently being animated
        return TRUE;
    }
    return FALSE;
}
...
static void TryPlayStatChangeAnimation(struct BattleCalcValues *cv, struct StatChange *st)
{
    // A spread (both-battlers) animation already covered this stat change.
    if (sStatChangeAnimsBatched)
        return;

    // HYDRA - a both-foes spread anim from an earlier spread target's pass already covered this
    // move + direction (see WillSpreadSecondaryStatHitPartner). Suppress the partner's own anim.
    if ((st->stage <= -1 && gBattleStruct->spreadStatAnimNeg)
     || (st->stage >= 1 && gBattleStruct->spreadStatAnimPos))
        return;
    ...
    // If the target's partner is getting the same-direction change (spread move like
    // Leer), play a single animation that draws on both mons at once, and suppress the
    // remaining per-target animations for this stat change.
    if (WillStatChangeAffectBattler(cv, BATTLE_PARTNER(cv->battlerDef), goesDown)
     || WillSpreadSecondaryStatHitPartner(cv, st, goesDown)) //HYDRA - per-target spread secondary drops
    {
        statAnimId |= STAT_ANIM_MULTIPLE_BATTLERS;
        sStatChangeAnimsBatched = TRUE;
        if (goesDown) //HYDRA - remember so the partner foe's later pass suppresses its own anim
            gBattleStruct->spreadStatAnimNeg = TRUE;
        else
            gBattleStruct->spreadStatAnimPos = TRUE;
    }
...
// Read-only: returns TRUE if `battler` has a pending stat change in `goesDown`'s
// direction that will actually apply. Used to decide whether a spread stat change
// should animate the target and its partner together.
static bool32 WillStatChangeAffectBattler(struct BattleCalcValues *cv, enum BattlerId battler, bool32 goesDown)
{
    if (battler >= gBattlersCount || !IsBattlerAlive(battler) || gSpecialStatuses[battler].statStageAmount == 0)
        return FALSE;

    // A target that avoided the move (accuracy miss) won't get the stat change.
if (gBattleStruct->moveResultFlags[battler] & MOVE_RESULT_MISSED)
        return FALSE;

    struct StatChange probe = {0};
    probe.onlyChecking = TRUE;
    probe.statStageQueue = gSpecialStatuses[battler].statStageQueue;
    probe.statStageAmount = gSpecialStatuses[battler].statStageAmount;

    enum BattlerId savedDef = cv->battlerDef;
    cv->battlerDef = battler;
    bool32 result = FALSE;

    for (u32 i = 0; i < probe.statStageAmount; i++)
    {
        enum Stat stat = probe.statStageQueue[i].stat;
        s32 stage = probe.statStageQueue[i].stage;
        ...
        probe.stat = stat;
        probe.stage = stage;
        AdjustStatStage(cv, &probe);
        stage = probe.stage;

        if ((stage < 0) != (goesDown != 0))
            continue; // different direction than the animation being played

        if (stage < 0)
        {
            if (CompareStat(battler, stat, MIN_STAT_STAGE, CMP_EQUAL, ABILITY_NONE))
                continue;
            if (CanDecreaseStat(cv, &probe) == STAT_CHANGE_DIDNT_WORK)
                continue;
        }
        else
        {
...
void ClearStatChangeValues(void)
{
    ...
    sStatChangeAnimsBatched = FALSE;
    sCombinedStatMsgPlayed = FALSE;
}
