...
#include "hydra_hotbar.h"       //HYDRA grayscale-state guard for reflections
#include "hydra_party_panel.h" //HYDRA grayscale-state guard for reflections
...
static void UpdateGrassFieldEffectSubpriority(struct Sprite *, u8, u8) __attribute__((section(".iwram"), long_call)); //HYDRA IWRAM hot path
...
static void LoadObjectRegularReflectionPalette(struct ObjectEvent *objectEvent, struct Sprite *sprite)
{
    ...
    //HYDRA if grayscale is active when a reflection is (re)created, match the main sprite's grey
    // ramp palette instead of building a blue pond-filtered one, so it is grey from frame one.
    if (HydraHotbar_IsOpen() || HydraPartyPanel_IsOpen() || HydraPartyPanel_IsGrayApplied())
    {
...
static void UpdateObjectReflectionSprite(struct Sprite *reflectionSprite)
{
    ...
    //HYDRA while the hotbar / party-panel grayscale is active the main sprite sits on the grayscale
    // ramp; give its reflection the SAME palette (skip the blue pond/ice tint) so the reflection
    // greys with every other sprite instead of showing a blue-tinted grey (the wrong palette).
    if (HydraHotbar_IsOpen() || HydraPartyPanel_IsOpen() || HydraPartyPanel_IsGrayApplied())
    {
        reflectionSprite->oam.paletteNum = mainSprite->oam.paletteNum;
    }
    ...
    else if (IndexOfSpritePaletteTag(HIGH_BRIDGE_PAL_TAG) != reflectionSprite->oam.paletteNum)
    {
        u16 baseTag = GetSpritePaletteTagByPaletteNum(mainSprite->oam.paletteNum);
        u16 paletteTag = REFLECTION_PAL_TAG(baseTag, mainSprite->oam.paletteNum);
        u8 paletteNum = IndexOfSpritePaletteTag(paletteTag);
...
void UpdateShadowFieldEffect(struct Sprite *sprite)
{
    ...
    else
    {
        ...
        if (!objectEvent->active
         ...
         || MetatileBehavior_IsTallGrass(objectEvent->currentMetatileBehavior)  //HYDRA no shadow while standing in grass (the grass hides it anyway); removes an alpha-blended sprite per object -> big perf win with many objects
         || MetatileBehavior_IsLongGrass(objectEvent->currentMetatileBehavior))
        {
            FieldEffectStop(sprite, FLDEFF_SHADOW);
        }
    }
...
__attribute__((section(".iwram"), long_call)) void UpdateTallGrassFieldEffect(struct Sprite *sprite) //HYDRA IWRAM: runs per grass rustle every frame
{
    u8 metatileBehavior;
    u8 localId;
    u8 objectEventId;
    bool8 notFound; //HYDRA
    ...
    //HYDRA Cache the triggering object's id in data[6] (0 = uncached, stored as id+1) so we skip
    // the O(16) GetObjectEventIdByLocalIdAndMap scan every frame. That scan grew costlier as OWEs
    // filled the object-event array -- which is why more spawns made the grass rustle lag worse.
    if (sprite->data[6] != 0 && gObjectEvents[sprite->data[6] - 1].active
     && gObjectEvents[sprite->data[6] - 1].localId == localId
     && gObjectEvents[sprite->data[6] - 1].mapNum == mapNum
     && gObjectEvents[sprite->data[6] - 1].mapGroup == mapGroup)
    {
        objectEventId = sprite->data[6] - 1;
        notFound = FALSE;
    }
    else
    {
        notFound = TryGetObjectEventIdByLocalIdAndMap(localId, mapNum, mapGroup, &objectEventId);
        sprite->data[6] = notFound ? 0 : (u8)(objectEventId + 1);
    }
...
__attribute__((section(".iwram"), long_call)) void UpdateLongGrassFieldEffect(struct Sprite *sprite) //HYDRA IWRAM: runs per long-grass rustle every frame
{
    u8 metatileBehavior;
    u8 localId;
    u8 objectEventId;
    ...
    mapGroup = sprite->sMapGroup;
    metatileBehavior = MapGridGetMetatileBehaviorAt(sprite->sX, sprite->sY);
    //HYDRA cache the triggering object's id in data[6] to skip the per-frame O(16) scan (see UpdateTallGrassFieldEffect)
    if (sprite->data[6] != 0 && gObjectEvents[sprite->data[6] - 1].active
     && gObjectEvents[sprite->data[6] - 1].localId == localId
     && gObjectEvents[sprite->data[6] - 1].mapNum == mapNum
     && gObjectEvents[sprite->data[6] - 1].mapGroup == mapGroup)
...
__attribute__((section(".iwram"), long_call)) static void UpdateGrassFieldEffectSubpriority(struct Sprite *sprite, u8 elevation, u8 subpriority) //HYDRA IWRAM hot path
{
    u8 i;
    ...
    for (i = 0; i < OBJECT_EVENTS_COUNT; i ++)
    {
        ...
        if (objectEvent->active)
        {
            struct Sprite *linkedSprite = &gSprites[objectEvent->spriteId]; //HYDRA removed unused GetObjectEventGraphicsInfo() call (was pure waste, ran per-object per-rustle per-frame)
...
u32 FldEff_OWE_SpawnAnim(void)
{
    ...
    if (spriteId != MAX_SPRITES)
    {
        ...
        //HYDRA - override the template callback so the anim self-terminates and stays drawn above the OWE
        sprite->callback = WaitFieldEffectSpriteAnim;
        sprite->sWaitFldEff = FLDEFF_OW_ENCOUNTER_SPAWN_ANIM;
