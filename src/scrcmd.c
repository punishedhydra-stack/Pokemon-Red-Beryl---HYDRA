...
struct ObjectEvent *ScriptHideFollower(void)
{
    ...
    struct ObjectEvent *obj2 = GetFollower2Object();

    // Send follower 2 into its ball alongside follower 1 (same enter-ball movement).
    // Handled up front so it still hides in the edge case where follower 1 is already
    // gone; follower 1 below continues to drive the caller's wait / return value.
    if (obj2 != NULL && !obj2->invisible)
    {
        ClearObjectEventMovement(obj2, &gSprites[obj2->spriteId]);
        gSprites[obj2->spriteId].animCmdIndex = 0; // Reset start frame of animation
        ScriptMovement_StartObjectMovementScript(obj2->localId, obj2->mapGroup, obj2->mapNum, EnterPokeballMovement);
    }
...
bool8 ScrCmd_lockall(struct ScriptContext *ctx)
{
    ...
    else
    {
        ...
        struct ObjectEvent *followerObj2 = GetFollower2Object();
        FreezeObjects_WaitForPlayer();
        SetupNativeScript(ctx, IsFreezePlayerFinished);
        if (FlagGet(FLAG_SAFE_FOLLOWER_MOVEMENT)) // Unfreeze follower objects (conditionally)
        {
            if (followerObj)
    ...
    else
    {
        ...
        if (gObjectEvents[gSelectedObjectEvent].active)
        {
            ...
            if (gObjectEvents[gSelectedObjectEvent].localId == OBJ_EVENT_ID_FOLLOWER)
                followerObj = NULL;
            if (gObjectEvents[gSelectedObjectEvent].localId == OBJ_EVENT_ID_FOLLOWER_2)
                followerObj2 = NULL;
        }
        else
        {
...
bool8 ScrCmd_releaseall(struct ScriptContext *ctx)
{
    ...
    if (followerObject2 && gSprites[followerObject2->spriteId].data[1] == 0)
...
void BufferFirstLiveMonNickname(struct ScriptContext *ctx)
{
    ...
    GetMonData(GetTalkedToFollowerMon(), MON_DATA_NICKNAME, sScriptStringVars[stringVarIndex]);
...
bool8 ScrCmd_checkfieldmove(struct ScriptContext *ctx)
{
    ...
    //HYDRA If no mon knows the move but the HM is usable from the bag (badge earned
    // + HM in bag), fall back to the first compatible mon. This lets scripts that
    // rely on checkfieldmove -- overworld Surf, cutting trees, smashing rocks,
    // moving boulders -- use an HM without a mon having to learn it.
    if (gSpecialVar_Result == PARTY_SIZE && CanUseHMFieldMoveFromBag(fieldMove))
    {
        for (u32 i = 0; i < PARTY_SIZE; i++)
        {
            enum Species species = GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_SPECIES);
            ...
            if (!GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_IS_EGG)
             && CanLearnTeachableMove(species, move) == TRUE)
            {
                gSpecialVar_Result = i;
                gSpecialVar_0x8004 = species;
                break;
...
void PlayFirstMonCry(struct ScriptContext *ctx)
{
    ...
    PlayCry_Script(GetMonData(GetTalkedToFollowerMon(), MON_DATA_SPECIES), CRY_MODE_NORMAL);
}
...
bool8 ScrCmd_lockfortrainer(struct ScriptContext *ctx)
{
    ...
    else
    {
        if (gObjectEvents[gSelectedObjectEvent].active)
        {
            ...
            if (FlagGet(FLAG_SAFE_FOLLOWER_MOVEMENT)) // match lockall's follower handling
            {
                if (followerObj)
                    UnfreezeObjectEvent(followerObj);
                if (followerObj2)
