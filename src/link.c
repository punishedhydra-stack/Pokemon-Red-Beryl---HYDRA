...
#if !FREE_LINK //HYDRA gut: link/RFU core (Stage 3)
...
//     bool8 disconnected;
// } sLinkErrorBuffer = {};
// static EWRAM_DATA u16 sReadyCloseLinkAttempts = 0;
...
u32 LinkMain1(u8 *shouldAdvanceLinkState, u16 *sendCmd, u16 (*recvCmds)[CMD_LENGTH])
{
    ...
    // switch (gLink.state)
    // ... (keep all your commented code)
    
    *shouldAdvanceLinkState = 0;
    // retVal = gLink.localId;
    // ... (keep commented)

    return retVal2;   // Now returns 0 instead of uninitialized
}
...
#endif //HYDRA !FREE_LINK (link core removed; stubs+data in link_stubs.c)