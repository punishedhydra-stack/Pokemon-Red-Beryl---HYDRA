...
#include "rtc.h"
#include "time_tint.h" //HYDRA 24-hour per-hour day/night tint
...
#include "sound.h"
#include "hydra_radio.h" //HYDRA radio decoration: base music override
...
#include "hydra_party_panel.h" //HYDRA freeze TOD while the start-menu panel is open
...
u16 GetCurrLocationDefaultMusic(void)
{
    ...
    //HYDRA - radio decoration: in the player's own secret base with a radio placed, the chosen
    // track is the base's default music (plays on load, survives any music reset). MUS_DUMMY = off.
    music = HydraRadio_GetBaseMusicOverride();
    if (music != MUS_DUMMY)
        return music;
...
void UpdateTimeOfDay(void)
{
    ...
    // Skip time-of-day palette updates when the hotbar or start-menu panel is open
    if (HydraHotbar_IsOpen() || HydraPartyPanel_IsOpen() || HydraPartyPanel_IsGrayApplied()) //HYDRA also stay guarded while the keep-gray SAVE dialog is up
        return;
    ...
    //HYDRA 24-hour per-hour tint overrides the 4-anchor blend above for the main
    // day/night coloring. gTimeOfDay + altWeight set by the branches are preserved,
    // so encounters, fog strength, and the night window-swap keep working; only the
    // colors change. Interpolates smoothly between this hour and the next by minute.
    // Time source is TimeTint_GetEffectiveTime (fast preview when the debug knob is on).
    {
        u32 tintHours, tintMinutes;
        TimeTint_GetEffectiveTime(&tintHours, &tintMinutes);
        gTimeBlend.startBlend = TimeTint_GetHourBlend(tintHours);
        gTimeBlend.endBlend   = TimeTint_GetHourBlend((tintHours + 1) % 24);
        gTimeBlend.weight     = TimeTint_GetInterpWeight(tintMinutes);
    }
}
...
#if TIME_TINT_DEBUG_SECONDS_PER_HOUR //HYDRA fast day/night preview: refresh the tint every few frames
        gTimeUpdateCounter = TIME_TINT_DEBUG_SECONDS_PER_HOUR;
#else
        gTimeUpdateCounter = (SECONDS_PER_MINUTE * 60 / FakeRtc_GetSecondsRatio());
#endif
        UpdateTimeOfDay();
        TryUpdateZoneWeatherHourly(); //HYDRA live hourly zone weather while standing still
...
void CB2_ContinueSavedGame(void)
{
    ...
    //HYDRA This function finishes via CB2_ReturnToField(), the same path used when
    // closing a menu, which leaves followers visible. On a fresh boot they should
    // start hidden and emerge on the player's first step instead.
    gHydraHideFollowersOnLoad = TRUE;
...
static u32 GetLinkSendQueueLength(void)
{
#if FREE_LINK //HYDRA gLink/gRfu removed; no link queue in single-player
    return 0;
#else
    if (gWirelessCommType != 0)
