...
//HYDRA Fully unload a battle-anim sprite's gfx + palette by tag: frees the VRAM/palette AND clears
// the anim load-tracking (sAnimSpriteGfxTags/sAnimSpritePalTags) that TryLoadGfx/TryLoadPal consult.
// A persistent overlay (the in-battle rain) needs this so it will re-decompress after the battle
// screen is torn down and rebuilt (returning from the party/bag menu wipes VRAM but leaves the
// tracking stale, which otherwise renders the overlay as black blocks).
void BattleAnim_UnloadSpriteGfxAndPalByTag(u16 gfxTag, u16 palTag)
{
    FreeSpriteTilesByTag(gfxTag);
    ClearSpriteGfxIndex(gfxTag);
    if (palTag != TAG_NONE)
    {
        FreeSpritePaletteByTag(palTag);
        ClearSpritePalIndex(palTag);
    }
}
