...
//HYDRA TRUE if the action is already in the party menu list. Used to avoid listing
// an HM field move twice when the mon both knows it and also qualifies for the
// badge/bag-based access added below.
static bool32 PartyMenuActionListContains(u8 action)
{
    u8 k;
...
//HYDRA TRUE if the species learns the move by level up. Used for Teleport, which
// isn't a teachable (TM/tutor) move, so CanLearnTeachableMove alone misses it.
static bool32 SpeciesLearnsMoveByLevelUp(enum Species species, enum Move move)
{
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    //HYDRA Also offer HM field moves the mon could use WITHOUT knowing them, so a
    // Water type can Surf without wasting a move slot on the HM. Requires all of:
    //   - the move is an HM
    //   - you have the badge that unlocks it (same check the field/menu already use)
    //   - the mon is compatible with the HM (can learn it)
    //   - the HM is in your bag
    if (!GetMonData(&mons[slotId], MON_DATA_IS_EGG))
    {
        ...
        for (j = 0; j != FIELD_MOVES_COUNT; j++)
        {
            ...
            if (!IsMoveHM(move))
                continue;
            if (!IsFieldMoveUnlocked(j)) // have the badge (same check the field uses)
                continue;
            if (!CanLearnTeachableMove(species, move))
                continue;
            if (!CheckBagHasItem(GetTMHMItemIdFromMoveId(move), 1))
                continue;
            if (PartyMenuActionListContains(j + MENU_FIELD_MOVES)) // mon already knows it
                continue;

            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, j + MENU_FIELD_MOVES);
        }

        //HYDRA Teleport stays selectable for any mon whose species learns it, even
        // after it forgets the move (e.g. Abra can always Teleport). Teleport is not
        // an HM, so no badge or bag item is required -- just species compatibility.
        if ((SpeciesLearnsMoveByLevelUp(species, MOVE_TELEPORT) || CanLearnTeachableMove(species, MOVE_TELEPORT))
         && !PartyMenuActionListContains(FIELD_MOVE_TELEPORT + MENU_FIELD_MOVES))
        {
...
static void Task_SlideSelectedSlotsOffscreen(u8 taskId)
{
    ...
    // A slot is offscreen once it has cleared the right edge, OR its right-hand edge has
    // passed the left edge. These are the same bounds TryMovePartySlot uses.
    //
    // The original test was "position > 33" against a u16, which only detects the right
    // edge: a slot sliding LEFT goes negative, wraps to a huge unsigned value, and
    // satisfies "> 33" after just two tiles while still fully on screen. That never
    // mattered in the single layout, because every swap involves a width-18 right-column
    // box whose ~22-frame slide dominates the && . In the double layout slots 1 and 2 are
    // BOTH width 10 (see sDoublePartyMenuWindowTemplate), so both slide left, both
    // "finish" after two frames, and the animation collapses into a two-tile jerk.
    slotOffscreen[0] = (slidingSlotPositions[0] > 33) || (slidingSlotPositions[0] + tSlot1Width < 0);
    slotOffscreen[1] = (slidingSlotPositions[1] > 33) || (slidingSlotPositions[1] + tSlot2Width < 0);

    // Both slots have slid offscreen
    if (slotOffscreen[0] && slotOffscreen[1])
    {
        tSlot1SlideDir *= -1;
        tSlot2SlideDir *= -1;
        SwitchPartyMon();
...
static void Task_SlideSelectedSlotsOnscreen(u8 taskId)
{
    ...
    {
        // Redraw both boxes fully instead of only restoring their tilemaps.
        //
        // The slide ClearWindowTilemap()s both windows at its midpoint, and they stay
        // cleared for the entire return journey. PutWindowTilemap alone re-points the
        // tilemap but redraws no contents, so if anything disturbs the window during
        // that stretch the box is left blank with the BG showing through -- and stays
        // that way until the menu is rebuilt. RenderPartyMenuBox redraws the contents
        // and pushes them, so a box cannot be left empty here.
    ...
    else
    {
        ...
        else if (SetUpFieldMove(fieldMove) == TRUE)
        {
            ...
            case FIELD_MOVE_TELEPORT:
                //HYDRA Teleport opens the Fly map, then warps with a plain fade (no bird)
                gTeleportViaFlyMap = TRUE;
            ...
            case FIELD_MOVE_DIG:
                ...
                break;
            case FIELD_MOVE_FLY:
                gTeleportViaFlyMap = FALSE; //HYDRA ensure Fly always uses the bird animation
                gPartyMenu.exitCallback = CB2_OpenFlyMap;
                Task_ClosePartyMenu(taskId);
                break;
            default:
...
//HYDRA TRUE if the player can use an HM field move without a mon knowing it: the
// badge is earned, the HM is in the bag, and some party mon is compatible with it.
// Mirrors the access rule added to the party menu in SetPartyMonFieldSelectionActions,
// so field-effect setup agrees with what the menu offers. Also used by the overworld
// "press A at water to surf" path in field_control_avatar.c.
...
bool32 SetUpFieldMove_Surf(void)
{
    ...
    //HYDRA Also allow surfing when a compatible mon + the Surf HM in the bag + Balance
    // Badge qualify, even if no mon actually knows Surf (matches the party menu option).
    if ((PartyHasMonWithSurf() == TRUE || CanUseHMFieldMoveFromBag(FIELD_MOVE_SURF)) && IsPlayerFacingSurfableFishableWater() == TRUE)
    {
...
void ItemUseCB_BattleScript(u8 taskId, TaskFunc task)
{
    ...
    // HYDRA - a reusable vial with no charges can't be used in battle.
    if (IsVialItem(gSpecialVar_ItemId) && gSaveBlock1Ptr->refillablePotionCharges == 0)
    {
        gPartyMenuUseExitCallback = FALSE;
        PlaySE(SE_SELECT);
    ...
    else
    {
        ...
        PlaySE(SE_SELECT);
        if (IsVialItem(gSpecialVar_ItemId)) //HYDRA - reusable vial: spend a charge, keep the item
            gSaveBlock1Ptr->refillablePotionCharges--;
...
void ItemUseCB_Medicine(u8 taskId, TaskFunc task)
{
    ...
    // ADD THIS BLOCK: Check and consume charge for Refill Potion
    if (IsVialItem(item)) //HYDRA
    {
        if (gSaveBlock1Ptr->refillablePotionCharges == 0)
        {
            gPartyMenuUseExitCallback = FALSE;
            PlaySE(SE_SELECT);
            DisplayPartyMenuMessage(gText_WontHaveEffect, TRUE);
    ...
    else
    {
        ...
        {
            PlaySE(SE_USE_ITEM);
            // MODIFIED: Don't remove Refill Potion from bag
            if (!IsVialItem(item)) //HYDRA
                RemoveBagItem(item, 1);
            else //HYDRA - consume a charge only when the heal actually worked
                gSaveBlock1Ptr->refillablePotionCharges--;
        }
        else
        {
...
void CB2_ChooseMonToGiveItem(void)
{
    ...
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_DOUBLE, PARTY_ACTION_GIVE_ITEM, FALSE, PARTY_MSG_GIVE_TO_WHICH_MON, Task_HandleChooseMonInput, callback);
...
{
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_DOUBLE, PARTY_ACTION_GIVE_MAILBOX_MAIL, FALSE, PARTY_MSG_GIVE_TO_WHICH_MON, Task_HandleChooseMonInput, Mailbox_ReturnToMailListAfterDeposit);
}
...
{
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_DOUBLE, PARTY_ACTION_MOVE_TUTOR, FALSE, PARTY_MSG_TEACH_WHICH_MON, Task_HandleChooseMonInput, CB2_ReturnToFieldContinueScriptPlayMapMusic);
}

void ChooseMonForWirelessMinigame(void)
{
    InitPartyMenu(PARTY_MENU_TYPE_MINIGAME, PARTY_LAYOUT_DOUBLE, PARTY_ACTION_MINIGAME, FALSE, PARTY_MSG_CHOOSE_MON_OR_CANCEL, Task_HandleChooseMonInput, CB2_ReturnToFieldContinueScriptPlayMapMusic);
}
