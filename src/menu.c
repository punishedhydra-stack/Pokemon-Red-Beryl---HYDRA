...
u8 AddMapNamePopUpWindow(void)
{
    if (sMapNamePopupWindowId == WINDOW_NONE)
    {
        ...
        else if (IsItemPopupActive())
            sMapNamePopupWindowId = AddWindowParameterized(0, 15, 1, 14, 2, 15, 0x107);
        else
...
//HYDRA hide the current menu cursor (used when the party panel takes focus)
void Menu_EraseCursor(void)
{
...
//HYDRA re-show the menu cursor at its current position
void Menu_RedrawCursor(void)
{
    RedrawMenuCursor(sMenu.cursorPos, sMenu.cursorPos);
    CopyWindowToVram(sMenu.windowId, COPYWIN_GFX);
}
...
void BufferSaveMenuText(u8 textId, u8 *dest, u8 color)
{
    ...
    *(string++) = color + 1;
    ...
    case SAVE_MENU_BADGES:
        ...
        *endOfString = EOS;
        break;
	case SAVE_MENU_LEVEL_CAP: //HYDRA
        string = ConvertIntToDecimalStringN(string, GetCurrentLevelCap(), STR_CONV_MODE_LEFT_ALIGN, 3);
        *string = EOS;
        break;
    }
}
