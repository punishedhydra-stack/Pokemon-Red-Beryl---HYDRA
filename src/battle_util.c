...
u16 gStolenItems[PARTY_SIZE] = {ITEM_NONE};
u8 gStolenItemCount = 0;
u16 gStolenItemToAnnounce = ITEM_NONE;
...
//HYDRA Weathers drawn as a continuous in-battle overlay (see battle_weather.c) suppress
// the per-turn "<weather> continues" message + one-shot animation, since the overlay
// already shows it on screen. Turn tracking (the --weatherDuration below) and the
// "<weather> stopped" end message are handled separately and are unaffected.
// Add sandstorm/snow/hail/fog cases here as they gain continuous overlays.
static bool32 IsWeatherShownContinuously(u32 battleWeather)
{
    switch (battleWeather)
    {
    //HYDRA suppress the per-turn "weather continues" message + its animation for all the standard
    // weathers (not just rain) so none of them spam a textbox every turn. The ongoing in-battle
    // visual/tint is driven separately by gBattleWeather and stays.
...
bool32 EndOrContinueWeather(void)
{
    ...
    {
        if (IsWeatherShownContinuously(currBattleWeather)) //HYDRA overlay shows it -> skip per-turn message + anim (duration already decremented above)
            return FALSE;
    ...
    case FIELD_EFFECT_OVERWORLD_WEATHER:
        if (!(gBattleTypeFlags & BATTLE_TYPE_RECORDED))
        {
            ...
            case WEATHER_DOWNPOUR:
                ...
                {
                    //HYDRA the in-battle rain overlay shows the rain itself, so set the weather for
                    // mechanics but DON'T announce it ("It's raining!") or play the intro weather
                    // animation. Leaving effect FALSE skips the OverworldWeatherStarts script, which
                    // also stops that animation from blanking our overlay at battle start.
                    gBattleWeather = B_WEATHER_RAIN_NORMAL;
                }
                break;
            case WEATHER_SANDSTORM:
                //HYDRA the in-battle overlay shows the sandstorm, so set the weather for mechanics but
                // DON'T announce it ("a sandstorm is raging") or play the intro anim -- leaving effect
                // FALSE skips OverworldWeatherStarts, exactly like the rain case above.
                if (!(gBattleWeather & B_WEATHER_SANDSTORM))
                    gBattleWeather = B_WEATHER_SANDSTORM;
                break;
            case WEATHER_DROUGHT:
                //HYDRA overlay/tint shows the sun -> set weather, no announcement (like rain).
                if (!(gBattleWeather & B_WEATHER_SUN))
                    gBattleWeather = B_WEATHER_SUN_NORMAL;
                break;
            case WEATHER_HAIL:
                //HYDRA the new overworld hail weather -> in-battle hail, no announcement (like rain).
                if (!(gBattleWeather & B_WEATHER_ICY_ANY))
                    gBattleWeather = B_WEATHER_HAIL;
                break;
            case WEATHER_SNOW:
                //HYDRA overlay shows the snow/hail -> set weather, no announcement (like rain).
                if (!(gBattleWeather & B_WEATHER_ICY_ANY))
                {
...
void TryRestoreHeldItems(void)
{
    ...
    // Process tracked stolen items
    for (i = 0; i < gStolenItemCount; i++) {
        AddBagItem(gStolenItems[i], 1);
        gStolenItemToAnnounce = gStolenItems[i];
    }
    
    // Clear for next battle
    gStolenItemCount = 0;

    // Restore original held items
