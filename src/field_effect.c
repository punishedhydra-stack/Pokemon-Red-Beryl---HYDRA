...
#include "battle_transition.h" //HYDRA reuse the pre-battle transition as the teleport effect
...
{
    //HYDRA Grass-rustle optimization. The player AND each follower spawn a tall/long grass
    // rustle every step; when running you leave the tile before the anim ends, so all
    // rustles stop and FieldEffectFreeGraphicsResources frees the sheet -- then the next
    // step re-allocs and re-copies it (x3 with two followers), which is the running lag.
    // Keep the tiny grass tile sheet RESIDENT so FieldEffectScript_LoadTiles reuses it
    // (it only reloads when the tag is unloaded). We still free the palette (cheap to
    // reload, no sprite-palette-slot leak). Nothing about the animation changes.
    if (id == FLDEFF_TALL_GRASS || id == FLDEFF_LONG_GRASS)
    {
...
bool8 gTeleportViaFlyMap = FALSE; //HYDRA

//HYDRA Re-sync the player's elevation with the tile they just landed on.
//HYDRA Warping to raw x/y (as the secret-base destination does) leaves elevation unset, so the
//HYDRA game picks the wrong sprite priority and a tree's upper layer draws over the player's head.
//HYDRA Walking a step would normally fix it; this does the same recalculation on arrival.
static void HydraFixPlayerElevationAfterWarp(void)
{
    struct ObjectEvent *player = &gObjectEvents[gPlayerAvatar.objectEventId];

    ObjectEventUpdateElevation(player, &gSprites[player->spriteId]);
}

//HYDRA Blink cadence shared by the teleport-out and teleport-in animations
#define HYDRA_TELEPORT_BLINK_SPEED 4 //HYDRA frames between visibility toggles
#define HYDRA_TELEPORT_BLINK_COUNT 8 //HYDRA number of toggles (~4 on/off blinks)
#define HYDRA_TELEPORT_RECALL_TIMEOUT 60 //HYDRA safety cap on waiting for the follower recall

//HYDRA TRUE if this object is a following Pokemon - ANY of them, not just the first.
//HYDRA Identified by behaviour (an overworld Pokemon object set to follow the player) so it works
//HYDRA regardless of how many followers exist or what local IDs they were given.
static bool32 HydraIsFollowingMon(struct ObjectEvent *obj)
{
    return obj->active
        && IS_OW_MON_OBJ(obj)
        && obj->movementType == MOVEMENT_TYPE_FOLLOW_PLAYER;
}

//HYDRA Recall every following Pokemon. Vanilla HideFollowerForFieldEffect() uses GetFollowerObject(),
//HYDRA which only ever returns the first follower - that left the second one standing on the map.
static void HydraRecallAllFollowers(void)
{
    u32 i;
    ...
    for (i = 0; i < OBJECT_EVENTS_COUNT; i++)
    {
        ...
        if (!HydraIsFollowingMon(obj) || obj->invisible)
            continue;

        ClearObjectEventMovement(obj, &gSprites[obj->spriteId]);
        ObjectEventSetHeldMovement(obj, MOVEMENT_ACTION_ENTER_POKEBALL);
    }
}

//HYDRA TRUE while any following Pokemon is still visible, finishing its return-to-ball animation
static bool32 HydraAnyFollowerStillVisible(void)
{
    u32 i;

    for (i = 0; i < OBJECT_EVENTS_COUNT; i++)
    {
        if (HydraIsFollowingMon(&gObjectEvents[i]) && !gObjectEvents[i].invisible)
            return TRUE;
    }
    return FALSE;
}

//HYDRA Blinks the player IN at the destination, playing the Teleport SE in full
static void Task_TeleportBlinkIn(u8 taskId)
{
    struct Task *task = &gTasks[taskId];

    if (gPaletteFade.active) //HYDRA wait until the destination map has faded in
        return;

    if (task->data[2] == 0) //HYDRA play the Teleport sound once, as the blinking begins
    {
        PlaySE(SE_M_TELEPORT);
        task->data[2] = 1;
    }

    if (task->data[0]++ % HYDRA_TELEPORT_BLINK_SPEED == 0) //HYDRA toggle every few frames
    {
        SetPlayerVisibility((task->data[1] & 1) != 0); //HYDRA start hidden, flicker into view
        task->data[1]++;
    }

    //HYDRA finish only after the full blink AND after the sound has finished playing
    if (task->data[1] > HYDRA_TELEPORT_BLINK_COUNT && !IsSEPlaying())
    {
        SetPlayerVisibility(TRUE);
        HydraFixPlayerElevationAfterWarp(); //HYDRA correct layering at the destination tile
...
//HYDRA Runs on the destination map: fade in, then blink the player in
static void FieldCallback_TeleportBlinkIn(void)
{
    ...
    FreezeObjectEvents();
    CreateTask(Task_TeleportBlinkIn, 0);
    gFieldCallback = NULL;
}

//HYDRA Squish confirmed working in-game; leave on.
#define HYDRA_TELEPORT_SQUISH 1

//HYDRA How long the psychic background plays BEFORE the player starts squishing, and how long the
//HYDRA squish holds before he launches. Both in frames - tune to taste.
#define HYDRA_TELEPORT_PSYCHIC_LEAD 24
#define HYDRA_TELEPORT_SQUISH_TIME  18

//HYDRA Which pre-battle transition to play when teleporting. Any B_TRANSITION_* works; it covers the
//HYDRA whole screen on top of everything and cleans itself up. Swap this one line to change the effect.
//HYDRA "Team Rocket"-style face-slam transitions: B_TRANSITION_MUGSHOT, B_TRANSITION_AQUA, B_TRANSITION_MAGMA.
#define HYDRA_TELEPORT_TRANSITION B_TRANSITION_HYDRA_PSYCHIC

//HYDRA Squish INWARD on X, and stretch slightly TALLER on Y (like he's being compressed and
//HYDRA squeezed upwards). Negative X delta = narrower, positive Y delta = taller.
//HYDRA Y: +1 per frame over 16 frames = scale 0x110 (~1.06x), i.e. about 2px taller on a 32px sprite.
//HYDRA Raise the Y value for a more pronounced stretch.
static const union AffineAnimCmd sHydraTeleportSquish[] =
{
    AFFINEANIMCMD_FRAME(-12, 1, 0, 16),
    AFFINEANIMCMD_END,
};
static const union AffineAnimCmd *const sHydraTeleportSquishTable[] = { sHydraTeleportSquish };

//HYDRA New teleport-out flow (arrival is unchanged): recall followers, squish the player in from the
//HYDRA sides, then play a real pre-battle transition (which covers the screen), then warp.
static void Task_TeleportBlinkOut(u8 taskId)
{
    struct Task *task = &gTasks[taskId];
    UNUSED struct Sprite *sprite = &gSprites[gPlayerAvatar.spriteId];

    switch (task->data[3])
    {
    case 0: //HYDRA wait for the origin map to fade in, then start the follower recall
        if (gPaletteFade.active)
            return;
        HideFollowerForFieldEffect();
        HydraRecallAllFollowers();
        ...
        task->data[3] = 1;
        break;
    case 1: //HYDRA wait for the recall to finish (timeout so it can never stall)
        if (FindTaskIdByFunc(Task_HideNPCFollowerAfterMovementFinish) != TASK_NONE)
            return;
        task->data[4]++;
        if (HydraAnyFollowerStillVisible() && task->data[4] < HYDRA_TELEPORT_RECALL_TIMEOUT)
            return;
        task->data[3] = 10;
        break;
    case 10: //HYDRA field move pose - player holds up the Poke Ball (same as FlyOutFieldEffect_FieldMovePose)
        {
            struct ObjectEvent *playerObj = &gObjectEvents[gPlayerAvatar.objectEventId];

            if (!ObjectEventIsMovementOverridden(playerObj) || ObjectEventClearHeldMovementIfFinished(playerObj))
            {
                task->data[6] = gPlayerAvatar.flags; //HYDRA remember the avatar state (surf/bike/etc)
        ...
        break;
    case 11: //HYDRA once the pose is struck, show the Pokemon (same as FlyOutFieldEffect_ShowMon)
        {
            struct ObjectEvent *playerObj = &gObjectEvents[gPlayerAvatar.objectEventId];
            ...
            if (ObjectEventClearHeldMovementIfFinished(playerObj))
            {
                ...
                if (monId > PARTY_SIZE - 1) //HYDRA same clamp Task_UseFly applies
                    monId = 0;
        ...
        break;
    case 12: //HYDRA wait for the Pokemon cameo to finish, then begin the psychic background
        if (FieldEffectActiveListContains(FLDEFF_FIELD_MOVE_SHOW_MON))
            return;

        //HYDRA Drop the Poke-Ball-holding field move pose and go back to the normal idle sprite
        //HYDRA BEFORE the squish, so he is squished as his normal self rather than mid-pose.
        //HYDRA Same restore FlyInFieldEffect_End uses. Done here, before the priority/subsprite
        //HYDRA settings below, because changing graphics resets some of those sprite fields.
        {
            struct ObjectEvent *playerObj = &gObjectEvents[gPlayerAvatar.objectEventId];

            ObjectEventSetGraphicsId(playerObj, GetPlayerAvatarGraphicsIdByStateId(PLAYER_AVATAR_STATE_NORMAL));
            ObjectEventTurn(playerObj, DIR_SOUTH); //HYDRA keep him facing the same way
        }

        CameraObjectFreeze();
        gPlayerAvatar.preventStep = TRUE;
        PlaySE(SE_M_TELEPORT);
        //HYDRA Draw the player ON TOP of the psychic background. The background sits on BG0 at
        //HYDRA priority 0; a sprite at the same priority wins over a background, so forcing the
        //HYDRA player to priority 0 keeps him visible for the whole transition. fixedPriority stops
        //HYDRA the per-frame object update from resetting it.
        gObjectEvents[gPlayerAvatar.objectEventId].fixedPriority = TRUE;
        sprite->oam.priority = 0;
        sprite->subspriteMode = SUBSPRITES_IGNORE_PRIORITY;
        //HYDRA psychic background plays FIRST, the player squishes after it is established
        BattleTransition_StartOnField(HYDRA_TELEPORT_TRANSITION);
        task->data[0] = 0;
        task->data[3] = 2;
        break;
    case 2: //HYDRA let the psychic background play for a beat, then start the squish
        if (++task->data[0] >= HYDRA_TELEPORT_PSYCHIC_LEAD)
        {
        #if HYDRA_TELEPORT_SQUISH
            //HYDRA AFFINE_DOUBLE is required now that he grows taller - AFFINE_NORMAL renders inside the
            //HYDRA original bounds and would just clip his head/feet instead of stretching him.
            //HYDRA DOUBLE doubles the bounding box, which is what shifted him diagonally before, so
            //HYDRA recalculate the centre-to-corner vector to keep him anchored in place.
            //HYDRA VERIFIED CAUSE OF THE POSITION JUMP: the player is a subsprite sprite, and
            //HYDRA AddSubspritesToOamBuffer() does "baseX = oam->x - sprite->centerToCornerVecX",
            //HYDRA i.e. it subtracts centerToCornerVec straight back out (src/sprite.c). So the
            //HYDRA compensation CalcCenterToCornerVec makes for AFFINE_DOUBLE was being cancelled
            //HYDRA every frame. Turning subsprites OFF sends him down the normal path, where
            //HYDRA centerToCornerVec is actually honoured. oam.priority still applies on that path,
            //HYDRA so he stays drawn above the psychic background.
            sprite->subspriteMode = SUBSPRITES_OFF;
            sprite->oam.affineMode = ST_OAM_AFFINE_DOUBLE; //HYDRA squish in from the sides + stretch up
            sprite->affineAnims = sHydraTeleportSquishTable;
            InitSpriteAffineAnim(sprite);
            StartSpriteAffineAnim(sprite, 0);
            CalcCenterToCornerVec(sprite, sprite->oam.shape, sprite->oam.size, sprite->oam.affineMode);
            //HYDRA Silence the object-event sprite callback for the rest of the effect. Frame order is
            //HYDRA RunTasks() -> AnimateSprites() -> BuildOamBuffer(), so that callback runs AFTER this
            //HYDRA task every frame and was resetting centerToCornerVec, leaving the hardware drawing a
            //HYDRA doubled sprite with a non-doubled centre - the instant down-right jump. We drive his
            //HYDRA position by hand from here on (the rise already sets sprite->y directly).
        ...
        break;
    case 3: //HYDRA hold while the squish plays out, then start the rise
        if (++task->data[0] >= HYDRA_TELEPORT_SQUISH_TIME)
        {
            task->data[1] = 1; //HYDRA rise speed
            task->data[2] = 4; //HYDRA frames between speed-ups
            task->data[5] = 0; //HYDRA distance risen so far
            task->data[3] = 4;
        }
        break;
    case 4: //HYDRA shoot the player straight up over the psychic bg, then warp when the transition ends
        //HYDRA Accelerating rise, same curve as the vanilla teleport warp-out: start at 1px/frame and
        //HYDRA double the speed every 4 frames up to 8. Stops once he is clear of the screen.
        if (!sprite->invisible)
        {
            s16 oamY;

            sprite->y -= task->data[1];
            task->data[5] += task->data[1];
            if ((--task->data[2]) <= 0 && task->data[1] < 8)
            {
                task->data[2] = 4;
                task->data[1] <<= 1;
            }

            //HYDRA src/sprite.c computes:
            //HYDRA   oam.y = y + y2 + centerToCornerVecY (+ gSpriteCoordOffsetY if coordOffsetEnabled)
            //HYDRA and stores it in an 8-BIT field. The instant that result goes negative it wraps to
            //HYDRA ~224 and he reappears at the BOTTOM of the screen. So hide him using that exact
            //HYDRA expression rather than a distance travelled guess - the old check triggered far too
            //HYDRA late, well after the wrap had already happened.
            oamY = sprite->y + sprite->y2 + sprite->centerToCornerVecY;
            if (sprite->coordOffsetEnabled)
                oamY += gSpriteCoordOffsetY;
        ...
        {
            //HYDRA Force everything black BEFORE the map load. The load blanks the display and shows
            //HYDRA the backdrop colour (palette entry 0) for a few frames - if that is not black you
            //HYDRA get a grey flash between the fade-out and the destination fading in.
            ...
            gFieldCallback = FieldCallback_TeleportBlinkIn; //HYDRA arrival unchanged (fade in + blink in)
            DestroyTask(taskId);
        }
        break;
    }
...
//HYDRA Runs on the origin map after a Teleport destination is chosen
static void FieldCallback_TeleportBlinkOut(void)
{
    FadeInFromBlack();
    LockPlayerFieldControls();
    FreezeObjectEvents();
    CreateTask(Task_TeleportBlinkOut, 0);
    gFieldCallback = NULL;
}

void ReturnToFieldFromFlyMapSelect(void)
{
    //HYDRA Teleport blinks the player out/in with the Teleport SE; Fly keeps its bird animation
    if (gTeleportViaFlyMap)
    {
        gTeleportViaFlyMap = FALSE;
        SetMainCallback2(CB2_ReturnToField);
        gFieldCallback = FieldCallback_TeleportBlinkOut;
    }
    else
    {
        SetMainCallback2(CB2_ReturnToField);
...
static void SurfFieldEffect_Init(struct Task *task)
{
    LockPlayerFieldControls();
    FreezeObjectEvents();
    //HYDRA Recall BOTH followers at once. Vanilla HideFollowerForFieldEffect only
    // recalls the first, which left follower 2 to despawn a beat later.
    HydraRecallAllFollowers();
    gPlayerAvatar.preventStep = TRUE;
...
static void SurfFieldEffect_ShowMon(struct Task *task)
{
    ...
    {
        //HYDRA Skip the Pokemon emerging animation on Surf -- go straight to jumping
        // onto the water. JumpOnSurfBlob waits on FLDEFF_FIELD_MOVE_SHOW_MON, which we
        // never start here, so it proceeds immediately.
        // (Was: gFieldEffectArguments[0] = task->tMonId | SHOW_MON_CRY_NO_DUCKING;
        //       FieldEffectStart(FLDEFF_FIELD_MOVE_SHOW_MON_INIT);)
        task->tState++;
    }
}
...
#define tSubsprModeFly  data[3] //HYDRA saved subsprite mode while airborne
...
static void FlyOutFieldEffect_FlyOffWithBird(struct Task *task)
{
    if ((++task->tTimer) >= 10)
    {
        ...
        objectEvent->noShadow = TRUE;
        //HYDRA airborne: draw above tree/overlay layers so the player isn't clipped by a canopy
        task->tSubsprModeFly = gSprites[objectEvent->spriteId].subspriteMode;
...
static void FlyOutFieldEffect_End(struct Task *task)
{
    if (!gPaletteFade.active)
    {
        ...
        //HYDRA back to normal layering
        objectEvent->fixedPriority = FALSE;
        gSprites[objectEvent->spriteId].subspriteMode = task->tSubsprModeFly;
...
static void FlyInFieldEffect_BirdSwoopDown(struct Task *task)
{
    ...
    if (!ObjectEventIsMovementOverridden(objectEvent) || ObjectEventClearHeldMovementIfFinished(objectEvent))
    {
        ...
        objectEvent->invisible = FALSE;
        objectEvent->noShadow = TRUE;
        //HYDRA airborne: draw above tree/overlay layers so the player isn't clipped while descending
        task->tSubsprModeFly = gSprites[objectEvent->spriteId].subspriteMode;
        objectEvent->fixedPriority = TRUE;
        gSprites[objectEvent->spriteId].oam.priority = 1;
        gSprites[objectEvent->spriteId].subspriteMode = SUBSPRITES_IGNORE_PRIORITY;
...
static void FlyInFieldEffect_FieldMovePose(struct Task *task)
{
    ...
    if (GetFlyBirdAnimCompleted(task->tBirdSpriteId))
    {
        ...
        sprite = &gSprites[objectEvent->spriteId];
        objectEvent->inanimate = FALSE;
        //HYDRA landed - restore normal layering so trees correctly overlap the player again
        objectEvent->fixedPriority = FALSE;
        sprite->subspriteMode = task->tSubsprModeFly;
