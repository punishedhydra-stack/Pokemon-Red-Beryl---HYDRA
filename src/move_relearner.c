...
#include "hyper_summary_screen.h" //HYDRA return to HYPER PC summary after relearning
#include "script.h"
#include "sound.h"
#include "sprite.h"
...
static void Task_MoveRelearner_Quit(u8 taskId)
{
    ...
    //HYDRA When the relearner was launched from the HYPER PC summary, return to the
    // HYPER summary (its battle/contest moves page) instead of the normal summary or
    // the overworld. gInitialHyperSummaryScreenCallback stays set across the round-trip.
    // ShowHyperSummaryScreen handles box mons itself when tPartyIndex == PC_MON_CHOSEN
    // (it rebuilds from gSpecialVar_MonBoxId/gSpecialVar_MonBoxPos), so gParties is only
    // used for party mons.
    if (gInitialHyperSummaryScreenCallback != NULL)
    {
        if (gRelearnMode == RELEARN_MODE_PSS_PAGE_CONTEST_MOVES)
            ShowHyperSummaryScreen(SUMMARY_MODE_RELEARNER_CONTEST, gParties[B_TRAINER_PLAYER], gTasks[taskId].tPartyIndex, gPartiesCount[B_TRAINER_PLAYER] - 1, gInitialHyperSummaryScreenCallback);
        else
            ShowHyperSummaryScreen(SUMMARY_MODE_RELEARNER_BATTLE, gParties[B_TRAINER_PLAYER], gTasks[taskId].tPartyIndex, gPartiesCount[B_TRAINER_PLAYER] - 1, gInitialHyperSummaryScreenCallback);
    }
