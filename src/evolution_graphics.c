...
static void Task_Sparkles_SpiralUpward(u8 taskId)
{
    if (gTasks[taskId].tTimer < 22)  // Changed from 64 to 22 (3x faster)
    {
...
static void Task_Sparkles_ArcDown(u8 taskId)
{
    if (gTasks[taskId].tTimer < 32)  // Changed from 96 to 32 (3x faster)
    {
...
static void Task_Sparkles_CircleInward(u8 taskId)
{
    if (gTasks[taskId].tTimer < 16)  // Changed from 48 to 16 (3x faster)
    {
        if (gTasks[taskId].tTimer == 0)
        {
            u8 i;
        ...
        if (gTasks[taskId].tTimer == 11)  // Changed from 32 to 11 (3x faster)
        {
            u8 i;
            for (i = 0; i < 16; i++)
...
static void Task_Sparkles_SprayAndFlash(u8 taskId)
{
    if (gTasks[taskId].tTimer < 43)  // Changed from 128 to 43 (3x faster)
    {
        u8 i;
        switch (gTasks[taskId].tTimer)
        {
        default:
            if (gTasks[taskId].tTimer < 17)  // Changed from 50 to 17 (3x faster)
        ...
        case 0:
            ...
            break;
        case 11:
			BeginNormalPaletteFade(0xFFFF041C, 0x05, 0x10, 0, RGB_WHITE);
			break;
		case 17:
			// Restore Pokemon palette 4 frames later
			CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(gSprites[gPostEvoSpriteId].oam.paletteNum)],
					&gPlttBufferFaded[OBJ_PLTT_ID(gSprites[gPostEvoSpriteId].oam.paletteNum)], 
					16 * sizeof(u16));
			CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(gSprites[gPreEvoSpriteId].oam.paletteNum)],
					&gPlttBufferFaded[OBJ_PLTT_ID(gSprites[gPreEvoSpriteId].oam.paletteNum)], 
					16 * sizeof(u16));
			break;
        }
        gTasks[taskId].tTimer++;
...
static void Task_CycleEvolutionMonSprite_Init(u8 taskId)
{
    ...
    gTasks[taskId].tScaleSpeed = 24;  // Changed from 8 to 24 (3x faster)
...
static void Task_CycleEvolutionMonSprite_TryEnd(u8 taskId)
{
    ...
    {
        gTasks[taskId].tScaleSpeed += 6;  // Changed from 2 to 6 (3x faster acceleration)
