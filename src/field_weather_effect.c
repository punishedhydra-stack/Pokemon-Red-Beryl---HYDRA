...
#include "overworld.h"
#include "random.h"
#include "rtc.h" //HYDRA dynamic weather: hour-of-day check
#include "script.h"
...
const u8 gWeatherHailTinyTiles[] = INCGFX_U8("graphics/weather/hail_tiny.png", ".4bpp");
const u8 gWeatherHailBreakTiles[] = INCGFX_U8("graphics/weather/hail_break.png", ".4bpp");
const u16 gWeatherHailPalette[] = INCBIN_U16("graphics/weather/hail.gbapal");
...
//HYDRA ==== Zone-based hourly weather ==========================================
// Hoenn is divided into 4 climate zones. On any outdoor map that belongs to a
// zone, the map's scripted header weather is replaced by a roll from that zone's
// probability table. The roll is a DETERMINISTIC function of (in-game day, hour,
// zone): identical every time within the same clock hour, so walking in/out of
// buildings, or between a town and a route in the same zone, never changes it --
// it only changes when the clock hour advances. Each zone rolls independently.
//
// Maps with no natural light (indoor/cave/underwater) and maps outside the 4
// zones keep their own weather. Scripted / coord weather that runs AFTER map load
// (Route 111 desert sandstorm, Route 113 ash) still overrides this on top.
enum
{
    ZONE_NORTH,   // Arid Highlands
    ZONE_WEST,    // Coastal Frontier
    ZONE_EAST,    // Urban & Mystic
    ZONE_SOUTH,   // Heartland
    ZONE_NONE,    // not part of any weather zone
};

//HYDRA last (day*24 + hour) value the zone weather was applied for; used by the
// live updater so it fires once per hour, not once per minute. EWRAM_DATA (matches
// this file's convention) so it lands in .data.ewram and isn't discarded at link.
EWRAM_DATA static u32 sLastWeatherHour = 0;

//HYDRA Per-zone odds, in percent. Each row MUST sum to 100. Tune freely here.
struct ZoneWeatherOdds
{
    u8 weather;
    u8 percent;
};

#define ZONE_WEATHER_SLOTS 5

static const struct ZoneWeatherOdds sZoneWeatherTable[4][ZONE_WEATHER_SLOTS] =
{
    [ZONE_NORTH] = // Clear 35 / Drought 30 / Rain 15 / Thunder 12 / Hail 8
    {
        { WEATHER_SUNNY,             35 },
        { WEATHER_DROUGHT,           30 },
        { WEATHER_RAIN,              15 },
        { WEATHER_RAIN_THUNDERSTORM, 12 },
        { WEATHER_HAIL,               8 },
    },
    [ZONE_WEST] = // Clear 40 / Rain 30 / Thunder 15 / Drought 10 / Hail 5
    {
        { WEATHER_SUNNY,             40 },
        { WEATHER_RAIN,              30 },
    ...
    [ZONE_EAST] = // Rain 35 / Clear 25 / Thunder 25 / Hail 10 / Drought 5
    {
        { WEATHER_RAIN,              35 },
        { WEATHER_SUNNY,             25 },
        { WEATHER_RAIN_THUNDERSTORM, 25 },
        { WEATHER_HAIL,              10 },
        { WEATHER_DROUGHT,            5 },
    },
    [ZONE_SOUTH] = // Clear 50 / Rain 25 / Thunder 15 / Drought 7 / Hail 3
    {
        { WEATHER_SUNNY,             50 },
        { WEATHER_RAIN,              25 },
        { WEATHER_RAIN_THUNDERSTORM, 15 },
...
//HYDRA Which weather zone (if any) a map-section belongs to.
static u8 GetWeatherZone(u16 mapSec)
{
    switch (mapSec)
    {
    // Zone 1: North Hoenn (Arid Highlands)
    case MAPSEC_ROUTE_111: // desert map -- keeps its scripted sandstorm; zoned to North otherwise
    case MAPSEC_ROUTE_112:
    case MAPSEC_ROUTE_113: // ash map -- keeps its coord-event ash; zoned to North otherwise
    ...
    case MAPSEC_LAVARIDGE_TOWN:
        return ZONE_NORTH;
    // Zone 2: West Hoenn (Coastal Frontier)
    ...
    case MAPSEC_SLATEPORT_CITY: // same zone as Route 109
        return ZONE_WEST;
    // Zone 3: East Hoenn (Urban & Mystic)
    case MAPSEC_ROUTE_118: // same zone as Route 119
...
//HYDRA TRUE for the five weather types the zone system can produce. The live
// updater uses this so it never overwrites an active scripted/coord special
// weather (sandstorm, ash, fog...).
static bool32 IsZoneWeather(u8 weather)
{
    return (weather == WEATHER_SUNNY
...
//HYDRA A single value identifying the current in-game hour (day * 24 + hour).
static u32 ZoneHourNow(void)
{
    u32 hour;

    RtcCalcLocalTime();
    hour = (gLocalTime.hours >= 0 && gLocalTime.hours < 24) ? (u32)gLocalTime.hours : 12;
    return (u32)gLocalTime.days * 24u + hour;
}

//HYDRA Deterministic 0..99 roll, fixed for a given (hour, zone). Folding the zone
// into the hash gives every zone its own independent weather each hour.
static u32 ZoneHourlyRoll(u8 zone)
{
    u32 h = (ZoneHourNow() + (u32)(zone + 1) * 0x9E3779B1u) * 2654435761u;
    h ^= h >> 15;
    return h % 100u;
}

//HYDRA Map a 0..99 roll to a weather via the zone's cumulative odds.
static u8 PickZoneWeather(u8 zone, u32 roll)
{
    u32 i, acc = 0;
    u8 weather = WEATHER_SUNNY;

    for (i = 0; i < ZONE_WEATHER_SLOTS; i++)
    {
        acc += sZoneWeatherTable[zone][i].percent;
        if (roll < acc)
        {
            weather = sZoneWeatherTable[zone][i].weather;
            break;
        }
    }

    // Drought (harsh sun) is a daytime-only effect; at night it falls back to clear.
    if (weather == WEATHER_DROUGHT && GetTimeOfDay() == TIME_NIGHT)
        weather = WEATHER_SUNNY;

    return weather;
}

//HYDRA Replacement for the map header weather on zoned outdoor maps.
static u8 RollZoneWeather(u8 mapWeather)
{
    u8 zone;

    if (!MapHasNaturalLight(gMapHeader.mapType)) // indoor / cave / underwater keep their own weather
        return mapWeather;

    zone = GetWeatherZone(gMapHeader.regionMapSectionId);
    if (zone == ZONE_NONE)                       // maps outside the 4 zones keep their header weather
        return mapWeather;

    return PickZoneWeather(zone, ZoneHourlyRoll(zone));
}

//HYDRA Called every in-game minute from the overworld loop. If the clock hour has
// advanced while the player stands on a zoned map, transition to the new hour's
// weather live. Skips maps currently showing a scripted/coord special weather.
void TryUpdateZoneWeatherHourly(void)
{
    u32 nowHour;
    u8 zone, current, pick;
    ...
    nowHour = ZoneHourNow();
    if (nowHour == sLastWeatherHour)
        return;
    sLastWeatherHour = nowHour;

    current = GetSavedWeather();
    if (!IsZoneWeather(current)) // don't clobber active sandstorm / ash / fog etc.
        return;

    pick = PickZoneWeather(zone, ZoneHourlyRoll(zone));
    if (pick != current)
    {
        SetSavedWeather(pick);
        SetNextWeather(pick); // smooth transition to the new hour's weather
    }
}
...
{
    u8 oldWeather = gSaveBlock1Ptr->weather;
    gSaveBlock1Ptr->weather = RollZoneWeather(TranslateWeatherNum(gMapHeader.weather)); //HYDRA zone weather
    UpdateRainCounter(gSaveBlock1Ptr->weather, oldWeather);
    sLastWeatherHour = ZoneHourNow(); //HYDRA remember this hour so the live updater doesn't immediately refire
}
...
static u8 TranslateWeatherNum(u8 weather)
{
	...
	case WEATHER_HAIL:               return WEATHER_HAIL;  // HYDRA
...
//------------------------------------------------------------------------------
// WEATHER_HAIL - Single tiny hail graphic with parallax scrolling
//------------------------------------------------------------------------------

#define GFXTAG_HAIL_BREAK 0x1204

static void UpdateHailSprite(struct Sprite *);
static void LoadHailSpriteSheets(void);
static bool8 CreateHailSprite(void);

static const struct OamData sHailOam =
{
    .affineMode = ST_OAM_AFFINE_OFF,
    .objMode = ST_OAM_OBJ_NORMAL,
    .bpp = ST_OAM_4BPP,
...
static const struct SpriteTemplate sHailSpriteTemplate =
{
    .tileTag = GFXTAG_HAIL_TINY,
    .paletteTag = PALTAG_WEATHER_3,
    .oam = &sHailOam,
    .anims = gDummySpriteAnimTable,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = UpdateHailSprite,
};

// Hail break animation data
static const struct OamData sHailBreakOamData =
{
    .y = 0,
    .affineMode = ST_OAM_AFFINE_OFF,
    .objMode = ST_OAM_OBJ_NORMAL,
...
// Animation commands - each shows a single frame for 8 frames (tile offsets: 0, 4, 8, 12)
static const union AnimCmd sHailBreakAnimCmd0[] = { ANIMCMD_FRAME(0, 8), ANIMCMD_END };
static const union AnimCmd sHailBreakAnimCmd1[] = { ANIMCMD_FRAME(4, 8), ANIMCMD_END };
static const union AnimCmd sHailBreakAnimCmd2[] = { ANIMCMD_FRAME(8, 8), ANIMCMD_END };
static const union AnimCmd sHailBreakAnimCmd3[] = { ANIMCMD_FRAME(12, 8), ANIMCMD_END };

static const union AnimCmd *const sHailBreakAnimCmds[] =
{
    sHailBreakAnimCmd0,
    sHailBreakAnimCmd1,
    sHailBreakAnimCmd2,
...
static void UpdateHailBreakSprite(struct Sprite *sprite)
{
    if (++sprite->data[0] > 8)
        DestroySprite(sprite);
}

static const struct SpriteTemplate sHailBreakSpriteTemplate =
{
    .tileTag = GFXTAG_HAIL_BREAK,
    .paletteTag = PALTAG_WEATHER_3,
    .oam = &sHailBreakOamData,
    .anims = sHailBreakAnimCmds,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = UpdateHailBreakSprite,
};

static void UpdateHailSprite(struct Sprite *sprite)
{
    ...
    case 1: // Hitting ground
        // 30% chance to show break animation
        if (Random() % 100 < 30)
        {
            // Spawn break animation sprite (centered on 8x8 hail)
            u8 breakSpriteId = CreateSpriteAtEnd(&sHailBreakSpriteTemplate, 
                sprite->x - 4, sprite->y - 4, 78);
            if (breakSpriteId != MAX_SPRITES)
            {
                // Pick random frame 0-3
                StartSpriteAnim(&gSprites[breakSpriteId], Random() % 4);
            }
            // Immediately respawn hail
            sprite->data[1] = 2;
            sprite->invisible = TRUE;
        }
        else if (++sprite->data[2] > 8)
        {
            sprite->data[1] = 2;
            sprite->invisible = TRUE;
        }
    ...
    case 2: // Respawn
        sprite->data[4] = -32;
        // sprite->data[3] = Random() % DISPLAY_WIDTH; //HYDRA REMOVED: re-randomizing world X each fall cycle reset the parallax anchor -> only a slight bend. Persistent anchor = whole field streams 1:1 (true parallax).
        // sprite->data[5] = Random() % DISPLAY_HEIGHT; //HYDRA REMOVED: same for vertical world anchor.
        sprite->data[0] = DISPLAY_HEIGHT + 32 + (Random() % 20);
        sprite->data[1] = 0;
        sprite->invisible = FALSE;
        break;
    }
    
    // HYDRA: horizontal world-lock parallax. screenX = worldX + camera offset, wrapped over the FULL screen width (period = DISPLAY_WIDTH = 240) so the whole field streams 1:1 opposite the player, seamless edges, no empty columns.
    s16 screenX = (sprite->data[3] + gSpriteCoordOffsetX) % DISPLAY_WIDTH;
    if (screenX < 0)
        screenX += DISPLAY_WIDTH;
    sprite->x = screenX;

    // HYDRA: vertical world-lock parallax + falling. data[5]+offsetY = world-locked phase, data[4] = fall. Clean wrap into [-32, DISPLAY_HEIGHT+32) (period DISPLAY_HEIGHT+64 = 224).
    s16 screenY = (sprite->data[5] + gSpriteCoordOffsetY + sprite->data[4]) % (DISPLAY_HEIGHT + 64);
    if (screenY < 0)
        screenY += DISPLAY_HEIGHT + 64;
    screenY -= 32;
    sprite->y = screenY;
}
...
static bool8 CreateHailSprite(void)
{
    u8 spriteId = CreateSpriteAtEnd(&sHailSpriteTemplate, 0, 0, 78);

    if (spriteId != MAX_SPRITES)
    {
        struct Sprite *sprite = &gSprites[spriteId];
        sprite->invisible = FALSE;
        // HYDRA: RANDOM world X (persistent). Irregular placement is what makes the horizontal parallax VISIBLE as the whole field sliding - evenly spaced stones made the (correct) motion look static. High sprite count keeps the gaps that random placement causes small.
        sprite->data[3] = Random() % DISPLAY_WIDTH;
        // HYDRA: random vertical phase across the full wrap period; falling smears vertical coverage.
        sprite->data[5] = Random() % (DISPLAY_HEIGHT + 64);
        // HYDRA: random initial fall progress so stones are desynced (no lockstep sheet).
        sprite->data[4] = -32 + (Random() % (DISPLAY_HEIGHT + 64));
        // Ground level below screen bottom
        sprite->data[0] = DISPLAY_HEIGHT + 32 + (Random() % 20);  // 192-212
        sprite->data[1] = 0;
        sprite->data[2] = 0;
        return TRUE;
    }
...
static void LoadHailSpriteSheets(void)
{
    if (GetSpriteTileStartByTag(GFXTAG_HAIL_TINY) == TAG_NONE)
    {
        struct SpriteSheet sheet = { gWeatherHailTinyTiles, sizeof(gWeatherHailTinyTiles), GFXTAG_HAIL_TINY };
        LoadSpriteSheet(&sheet);
        
        const struct SpritePalette hailPalette = {gWeatherHailPalette, PALTAG_WEATHER_3};
        LoadSpritePalette(&hailPalette);
    }
    
    // Load break animation tiles
    if (GetSpriteTileStartByTag(GFXTAG_HAIL_BREAK) == TAG_NONE)
    {
        struct SpriteSheet breakSheet = { gWeatherHailBreakTiles, sizeof(gWeatherHailBreakTiles), GFXTAG_HAIL_BREAK };
        LoadSpriteSheet(&breakSheet);
    }
}
...
void Hail_InitVars(void)
{
    ...
    gWeatherPtr->targetColorMapIndex = 3;
    gWeatherPtr->colorMapStepDelay = 20;
    gWeatherPtr->hailSpriteCount = 0;
    gWeatherPtr->targetHailSpriteCount = 20; //HYDRA density: random placement (needed for visible parallax) leaves gaps; more stones shrinks them. Kept <=~40 because MAX_SPRITES is 64 and overworld+break sprites share that budget
    SetRainStrengthFromSoundEffect(SE_RAIN);
...
void Hail_Main(void)
{
    ...
    {
    case 0:
        LoadHailSpriteSheets();
        gWeatherPtr->initStep++;
        break;
    case 1:
        if (gWeatherPtr->hailSpriteCount < gWeatherPtr->targetHailSpriteCount)
        {
            if (CreateHailSprite())
                gWeatherPtr->hailSpriteCount++;
        }
        else
        {
            gWeatherPtr->initStep++;
...
bool8 Hail_Finish(void)
{
    ...
    {
    case 0:
        if (gWeatherPtr->nextWeather == WEATHER_HAIL)
        {
            // Continue hailing if next weather is also hail
            gWeatherPtr->finishStep = 0xFF;
            return FALSE;
        }
        else
        {
            // Finish hail for any other weather transition
            gWeatherPtr->finishStep++;
        }
        // fall through
    case 1:
    {
        ...
        for (i = 0; i < MAX_SPRITES; i++)
        {
            if (gSprites[i].inUse && gSprites[i].template->tileTag == GFXTAG_HAIL_TINY)
                DestroySprite(&gSprites[i]);
            // Also destroy break sprites
            if (gSprites[i].inUse && gSprites[i].template->tileTag == GFXTAG_HAIL_BREAK)
                DestroySprite(&gSprites[i]);
        }
        FreeSpriteTilesByTag(GFXTAG_HAIL_TINY);
        FreeSpriteTilesByTag(GFXTAG_HAIL_BREAK);
    
        // Reset hail palette to black (or load default palette)
        u16 blackPalette[16] = {RGB_BLACK};
        LoadPalette(blackPalette, 0x100 + 14 * 16, 16);
