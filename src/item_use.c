...
//Refillable Potions - HYDRA
void ItemUseOutOfBattle_RefillablePotion(u8 taskId)
{
    if (gSaveBlock1Ptr->refillablePotionCharges == 0)
    {
        // Show "no charges" message and return to bag
        DisplayItemMessage(taskId, FONT_NORMAL, 
            COMPOUND_STRING("This vial has no charges\nleft. Rest at Mom's or a\nPokemon Center to refill."), //HYDRA
            CloseItemMessage);
    }
    else
    {
        // Use standard medicine flow - this will open party menu
        ItemUseOutOfBattle_Medicine(taskId);
    }
}

// HYDRA - Vial system helpers
bool32 IsVialItem(u32 item)
{
    return (item == ITEM_NURSES_VIAL || item == ITEM_SUPER_VIAL
         || item == ITEM_HYPER_VIAL || item == ITEM_MAX_VIAL
         || item == ITEM_MASTER_VIAL);
}

// Vial charge capacity depends only on badge count; 15 after the Elite 4. HYDRA
u32 GetVialMaxCharges(void)
{
    static const u8 sVialChargeCaps[NUM_BADGES + 1] = { 6, 8, 6, 9, 5, 7, 9, 9, 12 };
    u32 badgeCount = 0;
    u32 flag;
    ...
    return sVialChargeCaps[badgeCount];
}

// Owned vial tier: 0 = none, 1 = Nurse's, 2 = Super, 3 = Hyper, 4 = Max, 5 = Master. HYDRA
u32 GetOwnedVialTier(void)
{
    if (CheckBagHasItem(ITEM_MASTER_VIAL, 1))
        return 5;
    if (CheckBagHasItem(ITEM_MAX_VIAL, 1))
        return 4;
    if (CheckBagHasItem(ITEM_HYPER_VIAL, 1))
        return 3;
    if (CheckBagHasItem(ITEM_SUPER_VIAL, 1))
        return 2;
    if (CheckBagHasItem(ITEM_NURSES_VIAL, 1))
        return 1;
    return 0;
}

// One evolving vial: drop every other tier, keep exactly one of newVial,
// and fill charges to the new maximum. HYDRA
void OnVialAcquired(u32 newVial)
{
    static const u16 sVialItems[] = {
        ITEM_NURSES_VIAL, ITEM_SUPER_VIAL, ITEM_HYPER_VIAL, ITEM_MAX_VIAL, ITEM_MASTER_VIAL
    };
    u32 i;

    for (i = 0; i < ARRAY_COUNT(sVialItems); i++)
    {
        if (sVialItems[i] == newVial)
            continue;
        while (CheckBagHasItem(sVialItems[i], 1))
            RemoveBagItem(sVialItems[i], 1);
    }

    if (!CheckBagHasItem(newVial, 1))
    {
        AddBagItem(newVial, 1);
    }
    else
    {
        u16 owned = CountTotalItemQuantityInBag(newVial);
        if (owned > 1)
            RemoveBagItem(newVial, owned - 1);
    }

    gSaveBlock1Ptr->refillablePotionCharges = GetVialMaxCharges();
}

// Script special used by Mom's gift: fill the held vial to its max charges. HYDRA
void FillVialChargesToMax(void)
{
    gSaveBlock1Ptr->refillablePotionCharges = GetVialMaxCharges();
}
