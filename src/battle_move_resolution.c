...
static enum MoveEndResult MoveEndClearBits(struct BattleCalcValues *cv)
{
    ...
    gBattleStruct->spreadStatAnimPos = 0; //HYDRA - reset per-move spread stat-anim batching
    gBattleStruct->spreadStatAnimNeg = 0; //HYDRA
...
static enum MoveResult StatChangeTryChange(struct BattleCalcValues *cv)
{
    ...
    while (gBattleStruct->statChangeBattler < gBattlersCount)
    {
        ...
        {
            gBattleScripting.battler = cv->battlerDef; //HYDRA - name the battler that avoided, not the slot index
