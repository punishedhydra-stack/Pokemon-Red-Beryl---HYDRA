...
static void SpriteCB_PokemonLogoShine(struct Sprite *sprite)
{
    if (sprite->x < DISPLAY_WIDTH + 32)
    {
        ...
        if (sprite->sMode != SHINE_MODE_SINGLE_NO_BG_COLOR)
        {
            ...
            {
                gPlttBufferFaded[0] = RGB(19, 1, 10);
                gPlttBufferUnfaded[0] = RGB(19, 1, 10);
            }
            else
...
static void Task_TitleScreenPhase3(u8 taskId)
{
    ...
    else
    {
        ...
        UpdateLegendaryMarkingColor(gTasks[taskId].tCounter + 128);
...
static void UpdateLegendaryMarkingColor(u8 frameNum)
{
    if ((frameNum % 4) == 0) // Change color every 4th frame
    {
        ...
        u32 r = Q_8_8_TO_INT(intensity * 31);
        u32 g = Q_8_8_TO_INT(intensity * 31);
        u32 b = 0;
        u16 color = RGB(r, g, b);
