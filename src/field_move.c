...
//HYDRA weather moves aren't badge-gated; the hotbar gates them on owning the TM + a mon knowing the move.
static bool32 IsFieldMoveUnlocked_Weather(void)
{
    return TRUE;
}
...
const struct FieldMoveInfo gFieldMoveInfo[FIELD_MOVES_COUNT] =
{
    ...
    //HYDRA weather field moves
    [FIELD_MOVE_RAIN_DANCE] =
    {
        .fieldMoveFunc = SetUpFieldMove_RainDance,
        .isUnlockedFunc = IsFieldMoveUnlocked_Weather,
